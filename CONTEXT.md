...
**自定义 provider**:
操作员在面板上加进模型目录的一家 OpenAI-compatible 端点。名字由操作员起,限小写字母、数字与连字符,与 Pi 内置的那些家共用同一命名空间,撞名拒收。一个名字对应一个 base URL、一个接口协议与一把模型凭据;同一个端点要接两个 base URL 就起两个名字。base URL 与接口协议都是必填:全新的一家没有继承来源,缺任一者这家整个从目录里消失。
_Avoid_: 自建 provider、custom provider、私有厂商

**模型行**:
操作员在面板上手填进库的一个模型定义,落在一家已配模型凭据的 provider 下,最少只有一个 model id——接口协议与 base URL 由 Pi 从该家已有的模型继承。库是它的真相源,Pi 的用户模型配置由它整份派生,派生文件清掉也能重建。它与远程目录、厂商目录补进来的模型分落两层,互不覆盖。
_Avoid_: 手填模型、自定义模型、model row
