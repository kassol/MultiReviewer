...
- 产品页主区分「产品知识」与「产品 tracker」两个 tab(与阶段详情同一套 tab 语法),概览那一行留在 tab 之外——产品名与「…」菜单在哪一页都要够得着;当前 tab 记在 `?tab=` 上,缺省的产品知识不写进地址,`?spec=` 在即落 tracker。产品知识的标题行之下是一行工具:筛选框(`sm` 起封在 20rem、靠左,窄屏独占一行)与靠右的段内锚点,锚点跳到各段标题(`scroll-mt-[88px]` 让开顶栏)、不改地址;计数随筛选显示命中数,空段与单段不画锚点,五条以内的知识不画筛选框。
