...
**产品页与会话页共用同一个左栏,只换主区。**同一个组件、同一个 264px 宽、同一套卡片(产品列表、当前产品的仓库、会话)、同一套选中态:会话页里当前产品在产品列表高亮、当前会话在「会话」高亮。两页的写动作也是同一份(「建产品」「归属仓库」与每行的移出、改职责、「建会话」,各按自己的权限格显隐),回执落在各页顶部那条 Callout 里。左栏一字不差,跳过去就只是主区从「概览 + 产品知识」换成「聊天工作台(+ 右栏产出)」,不像换了个应用——与评审记录页「左栏仓库、右栏阶段」的主从结构同律。

左栏在两页的滚动锚点不同、视觉位置相同:会话页整屏不滚,左栏 `lg:h-full` 自己滚;产品页整页在 `#panel-main-scroll` 里滚,左栏 `lg:sticky lg:top-[88px]`(顶栏高度)加 `lg:max-h-[calc(100vh-112px)]` 自己滚,并 `lg:self-start` 让它不被 flex 拉满——否则 sticky 不生效。`lg` 以下两页都竖排:产品页依次是产品列表、概览 + 产品知识、仓库、会话——先选产品,再看它是什么,仓库与会话排到后面;左栏那个 `aside` 在这一档 `max-lg:contents` 让出自己的盒子,三张卡与主列因此同为一个 flex 容器的直接子项,靠 `max-lg:order-*` 排,`lg` 起 `aside` 恢复成一列(landmark 语义在桌面保留)。会话页左栏整块不显示,头部那行产品名链接是回产品的入口。
