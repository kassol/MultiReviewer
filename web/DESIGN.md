...
| `text-xl` | 16px | 1.45 | 面包屑当前页、命令面板结果项、登录表单、模态标题（Themes `Dialog.Title size="4"`） |
| `text-2xl` | 17px | 1.4 | 卡片区块标题、会话页的会话标题 |
| `text-3xl` | 19px | 1.35 | 抽屉标题（Finding 侧滑、审查轨迹、图的全屏预览） |
...
**不写 `text-[13px]` 这类一次性值。** 两处例外：移动端底部 Tab 栏的 11px 标签低于阶梯下界（Tab 栏一行要放五格，标签跟着阶梯走会把两字以上的页名挤断行）；行内 `code` 用相对值 `text-[0.9em]`（`Markdown` 与 `Statement` 两处同值），它嵌在别人的行里——正文、标题与元信息行的字号各不相同，钉死一档会让它在标题里显得过小、在小字里显得过大。
...
- `lg=1024px` 是首页与模型服务主从双栏的切换点，网格是 `264px minmax(0,1fr)`；产品页与会话页共用同一个 264px 左栏(见 7.5)。外壳仍在 `sm=640px` 切换；640–1023px 内容区继续使用列表／详情单层布局。
...
- 阶段详情的三个计数与 tab 栏吸在顶栏之下(`sticky top-[var(--v8-top-chrome)] z-20`,见 7.1 的顶边让位),页头其余部分(阶段名、来源与状态、动作)照常滚走。几百条的阶段里往下看了一段之后,切到时间线、改处置状态筛选都得先滚回页顶才够得着,而三个计数正是处置状态筛选的唯一入口。这一条实底(`bg-background`,内容从它底下滚过去,半透明会让卡片的字透出来)、`-mx-1 px-1` 比内容轨宽出 8px 让卡影一并盖住,下边线就是 `Tabs.List` 自带的那道 inset 阴影、不另画一条。层级压在顶栏(z-30)与侧滑(z-40 / z-50)之下。吸顶态不换一套更紧凑的排版:检测「吸没吸着」要另搭一个哨兵。`sm` 以下这一条压矮(issue #444):上留白 `pt-1`、两行间距 `gap-2`、计数键 `py-1`、计数数字 `text-xl`,内容一样不少,目标不超过 120px;计数键收完仍高于 44px,tab 的 44px 由 coarse 块给。`sm` 起一格不变。
- 阶段详情的 Finding 列表按滚动逐段渲染:首段 50 项,接近底部时追加一段,一张同根因组卡算一项(组内成员随组卡一起画)。尾部一行「已显示 N / M 条，向下滚动加载其余；页内查找只搜得到已显示的条目」,全部画完这一行就不出现。筛选与顶部三个计数仍对全量算;换筛选条件回到首段;列表第一张卡已经滚过吸顶工具条下沿时,只把它滚回贴着那条线,还没滚过就不动(issue #444)——滚回页顶那一下工具条先脱开再落回,整页跳一截。几百条的阶段一次画全要两秒、留下两万多个 DOM 节点,而人一屏看得到的只有头几条。
...
外壳下挂六页：`/` 评审记录、`/products` 产品、`/stats` 处置率、`/credentials` 模型服务、`/settings` 审查策略、`/access` 访问控制，另有 `/password` 与外壳外的 `/login`。仓库注册表没有自己的页面，它是首页左栏；导航顺序即 `NAV` 数组顺序，评审记录打头、账户项收尾。

**当前对象写在地址上,不写进组件 state**:产品页是 `/products/$productId`,会话页是 `/products/$productId/sessions/$sessionId`。`/products` 不带产品,当前项落在列表第一个上;带的产品这个账号看不到时同样落到第一个。从会话页回产品页因此选的还是同一个产品——选中态放进组件 state 的那一版,一跳就丢。

顶栏面包屑按真实路由层级取:导航项的页名之后,产品页多一段产品名,会话页再多一段会话用途。两段读的是产品详情与会话那两份缓存键,不另开端点;还没读到的那一段先不画。`lg` 以下这两段收起,只留品牌与页名。
...
**产品页与会话页共用同一个左栏,只换主区。**同一个组件、同一个 264px 宽、同一套卡片(产品列表、当前产品的仓库、会话)、同一套选中态:会话页里当前产品在产品列表高亮、当前会话在「会话」高亮。两页的写动作也是同一份(「建产品」「归属仓库」与每行的移出、改职责、「建会话」,各按自己的权限格显隐),回执落在各页顶部那条 Callout 里。左栏一字不差,跳过去就只是主区从「概览 + 产品知识 + 产品 tracker」换成「聊天工作台(+ 右栏那一场写下的 spec 与票)」,不像换了个应用——与评审记录页「左栏仓库、右栏阶段」的主从结构同律。

左栏在两页的滚动锚点不同、视觉位置相同:会话页整屏不滚,左栏 `lg:h-full` 自己滚;产品页整页在 `#panel-main-scroll` 里滚,左栏 `lg:sticky lg:top-[var(--v8-top-chrome)]`(顶栏实高,见 7.1 的顶边让位)加 `lg:max-h-[calc(100vh_-_var(--v8-top-chrome)_-_80px)]`(视口减顶栏再减 `PageBody` 的底部留白 `pb-20`:sticky 出不了父容器,滚到最底时父容器底边离视口底正好 80px,少减这一截左栏就被顶进顶栏底下,issue #444)自己滚,并 `lg:self-start` 让它不被 flex 拉满——否则 sticky 不生效。`lg` 以下两页都竖排:产品页依次是产品列表、会话、仓库、概览 + 产品知识 + 产品 tracker——先选产品,再进会话;会话是人在手机上要打开的那一样,概览与产品知识是读物,排到最后;左栏那个 `aside` 在这一档 `max-lg:contents` 让出自己的盒子,三张卡与主列因此同为一个 flex 容器的直接子项,靠 `max-lg:order-*` 排,`lg` 起 `aside` 恢复成一列(landmark 语义在桌面保留)。会话页左栏整块不显示,头部那行产品名链接是回产品的入口。**当前产品名在 `lg` 以下由页顶一行(`lg:hidden`)说**:那一档顶栏面包屑收起、主区又排在左栏三张卡之后,不补这一行的话人在 390px 上看不出自己正在哪个产品里;概览卡里的那一份同档让位(`max-lg:hidden`),同一个名字只画一遍。

产品页主区的两个区块:

- 折叠区(Radix Collapsible)的开合统一走 `styles.css` 的 `.collapsible-motion`:高度 160ms 出、120ms 收,`prefers-reduced-motion` 时不播;这是这两页唯一的布局动效,tab 切换不加。带滚动的弹窗内层滚动格要 `relative`,里面不放没有定位祖先的 `sr-only`——它会按外层定位把外层撑出第二条滚动条。
- 产品页主区分「产品知识」与「产品 tracker」两个 tab(与阶段详情同一套 tab 语法),概览那一行留在 tab 之外——产品名与「…」菜单在哪一页都要够得着;当前 tab 记在 `?tab=` 上,缺省的产品知识不写进地址,`?spec=` 在即落 tracker。产品知识的标题行之下是一行工具:筛选框(`sm` 起封在 20rem、靠左,窄屏独占一行)与靠右的段内锚点,锚点跳到各段标题(`scroll-mt-[var(--v8-top-chrome)]` 让开顶栏实高,`sm` 以下顶栏只剩一行,落点跟着贴上来)、不改地址;计数随筛选显示命中数,空段与单段不画锚点,五条以内的知识不画筛选框。
- **产品知识**分三段(术语表、仓库关系、产品决策)。陈述段落与 Markdown 正文同一档字号(`text-lg`)、行宽封在 `max-w-[46em]`——它们常常是整段话,铺满不设上限的内容轨之后眼睛要横跨整屏才回到行首;徽章行与出处行不限宽,它们是扫的,不是读的。术语表按主题分组,每组一个默认展开的 Collapsible 段头(组名 + 条数,没分组的那几条挂在「其余」下),组与组之间比组内两条之间松一档。条目按定义列表排:`lg` 起名字一列(13rem)、正文一列,三段的正文落在同一条左缘上,仓库关系没有名字、正文仍在第二列;产品决策的状态徽章在名字之下,被取代的那一条正文退到 `text-text-secondary`——不用彩色左边条(list item 上的色条是装饰,状态由徽章说)。
- **产品 tracker** 的计数带单位(「N 条 spec · M 张票」),不写成「N / M」——后者读着像进度。spec 行整行 hover 铺一层 `bg-sunken`:标题在左、「导出」在右端,宽屏上两头隔着几百像素,没有底色时看不出它们属于同一条;标题那颗 ghost 键静息态带一颗 chevron,hover 与 focus 下划线,否则它与纯文字无异。**打开的是哪一条 spec 记在地址的 `?spec=` 上**,与阶段详情的 `?finding=` 同一写法(`useRouterState` 读,开关都走 `replace`):会话页右栏「本会话写的 spec 与票」那几行链过来时带的就是它,产品页落地即展开那条的全文弹窗,刷新与分享链接打开的是同一条。票那几行不带这个参数——读会话那一份只给票的 id 与标题,它属于哪条 spec 服务端没回。
- spec 全文弹窗里**一张票一行**:标题那一行装齐编号、标签、状态、认领人、可开工标记与那几个动作(认领 / 改标签 / 开关),正文与评论进 Collapsible、点开才展开——一条 spec 常带十来张票,全摊开要滚半天才找得到要动的那一张;动作贴着标题那一行,收起时也要认领得了、改得了标签。行首那颗展开键是原生 `button`,触控高度自己给(`pointer-coarse:min-h-11`)。`sm` 以下标题独占一行(`basis-full`),元信息与动作折到下一行,不产生横向滚动。

Agent 会话页的中栏是一块占满视口的聊天工作台,整页不滚:`PageBody` 加 `h-full`,壳只在这条路由上给 `main` 加 `min-h-0`(其余页面 main 随内容长高、整体在 `#panel-main-scroll` 里滚,加了会让 sticky 的移动端 Tab 栏跟着滚走),高度沿 flex 链给下来,不写视口常量。头部一行细标题(起始一颗回产品的箭头键、会话标题 `text-2xl`(服务端从首条用户消息派生并截到 80 字,截断时面板补一个省略号;没有标题的会话显示用途名。左栏「我的会话」、这里与顶栏面包屑共用 `sessionTitle`)、在跑徽章、一行会话 id / 时刻 / 建立人 / 用量、每仓库一行的基点),元信息行开头是 `会话 #<id>`(只有 `#<id>` 走 `font-mono tabular-nums`,「会话」二字随正文字体——等宽字体里的汉字与空格会把间距撑开;全局 id,排障口径与日志、API 一致)。这一行是一段素文字,不做 flex:flex 的 gap 叠上字面空格会让「·」两侧间距不均;用量总数 `tabular-nums` 不换等宽字体,点线下划线用 `--v8-text-faint` 提示悬停有分项,对话流自己滚,输入框钉在底部。产品名不在左栏里再重复一张小卡:左栏的产品列表已经把当前产品高亮出来了。**`sm` 以下头部收成一行**:箭头键只剩图标(`sm` 起键上带产品名),标题单行截断(全文在 `title=`),标题行不许折行,时刻 / 建立人 / 用量与基点收进标题右侧那颗 disclosure,摊开才看——这三行元信息在 390px 上把对话流压到 57% 的屏。折叠态整块 53px:一行 ghost 键的 44px 加 `pb-2` 与分隔线。写下过 spec 与票的会话在这一档多一行:「对话 / spec 与票」分段控件让开标题行,整行铺在标题下面(`max-sm:order-last max-sm:w-full`,包着它与「更多操作」的那层壳同档换成 `contents`,两颗控件因此升成头部那层 flex 的直接子项),折叠态因此 105px——返回键、标题、162px 的分段控件与「更多操作」挤一行时标题只剩 27px(issue #388)。这一档页头留白也收到 16px、缺损横幅与头部之间收到 8px,省下的高度全给对话流;`sm` 起两处照常 24 / 12px。新条目来时人在底部(距底 ≤ 80px)就跟着滚,翻上去看旧消息时不打扰,右下角浮一颗 44px 高的「最新」胶囊送回底部;胶囊现身时对话流末尾多留一段,它因此不压最后一条消息。

**头部只占一行。**`lg` 起标题(单行截断,全文在 `title=`)、元信息行与基点摘要排在同一行,高约 37px;标题至少留这一行的 45%,有右栏时元信息行放不下就截断(会话号在最前,截不掉)。`lg` 以下沿用下面那颗 disclosure。

**轮次导航(仿 Codex)**:对话流左侧空白里一列刻度,一轮(一条用户消息到下一条之前)一根 2px 横线。正在读的那一轮(起头那条消息已经滚过视口上三分之一的最后一轮,到底即最后一轮)最长、`--v8-text` 实色;其余 `--v8-text-faint`;长度按离焦点(悬停或键盘聚焦的那一根,没有就是当前轮)的距离收短(28 / 20 / 15 / 12,再远 10px),像放大镜一样鼓起来,150ms ease-out,减少动态效果时不动画。指到或聚焦一根刻度,右侧浮出一张卡(`rounded-2xl` + `border-card-line` + `bg-surface` + `shadow-overlay`,320px 宽,不接指针):粗体一行是这一轮的问题(答卷写「回答提问轮次」,只有图片写「发了图片」),下面三行是这一轮最后一条回复的开头(去掉 Markdown 记号);点刻度平滑滚到那一轮。刻度是带「第 N 轮:问题」读屏名的按钮,当前轮 `aria-current="location"`。至少两轮才出现;只在中栏宽 ≥ 1048px(920px 对话列加两侧各 64px,容器查询)时出现——窄了它会压在正文上,因此窄屏与带右栏时都没有它。书签不做。
...
- **一轮对话是对话流的分段单位**:一轮从一条用户消息起,到下一条用户消息之前止。轮内各条之间是 `gap-3`,用户消息自己再多一档上边距(`mt-3`,首条不加)——一屏二三十条消息连着排时,看不出一次提问带出的那几条回复到哪里为止。只按条目类型加类名,不改记录的数据形状。
- 消息两种形态:人的消息是右对齐的 `bg-accent-tint` 气泡(`rounded-2xl rounded-br-md`,`max-w-[80%]`),长消息与 agent 长回复同一道阈值收到 320px(`mask-image` 渐隐,气泡下「展开 / 收起」),agent 的一条完整回复**不套卡**,正文直接落在对话列(页面底)上——一场会话几十条回复,每条一张白卡就是一面卡片墙,需要容器的是人的气泡、工具行、子代理卡与提问卡;流式那一版同样没有卡,落库后不跳。时刻与「展开 / 收起」「阅读」「复制 Markdown」放在一行 footer 里:`md` 以下与折叠态在卡底(折叠态的「展开」常显、贴正文左缘,时刻与其余动作靠右),`md` 起非折叠态这一行浮在这条回复的右上沿(发丝边小条、`shadow-control`,指到卡片才显,不占回复自己的高度;`sm` 以下这几颗只剩图标、文字留给读屏——带文字要 234px,卡里只有 226px,末一颗会被切掉;折叠态底部的渐隐收到页面底 `from-background`);超过 800 字或 12 个换行的长回复默认收到 320px 高、底部渐隐,「阅读」开 1200px 宽的阅读视图(尽量用宽度,正文铺满不另限宽;`Markdown` 的 `article` 档:正文 16px / 1.7。头部一行与「标题 + 时刻」整块垂直居中,头部不收缩、只有正文那一格滚,滚动条不贯穿头部)。时刻在 footer 左侧,`md` 以下常显、`md` 起指到才显(改 opacity,布局不动)。
...
- 会话子代理是对话流里的嵌套卡片,一趟一张:卡壳与图卡同一套令牌(`rounded-lg` + `border-card-line` + `bg-surface`,不带阴影——它嵌在对话里,不是浮层)。内边距与 agent 回复卡对齐(`px-4`)。卡头一行是展开箭头(在跑时换成 Spinner)、Person 图标、任务与右端的「在跑 · N 步」/「失败 · N 步」/「N 步」;**任务只显示人要读的那半句**——派单提示常常先交代一遍工作目录怎么铺的,「任务:」之后的才是要做的事,`lib/agent-session-records.ts` 的 `subagentTaskLabel` 裁掉前面那一段(没有这个标记时去掉开头那句「在…下,」),全文留在 `title` 上,裁不出东西时原样显示;展开是同一道 `--v8-border-line` 竖线下的工具调用行,与父会话共用 `ToolRow`,失败的那一步下面一行 `text-danger` 原因。结论在卡底,与卡头之间一道 `border-card-line` 分隔,`Markdown` 渲染;失败那一趟的结论整段 `text-danger`。**长结论与 agent 回复卡同一道阈值、同一种收法**(超过 800 字或 12 个换行即收到 320px 高、底部渐隐,卡底一颗「展开 / 收起」),不另立一套规则——子代理交回来的也会是一篇文档。跑完的卡片默认收着,看得见的就是结论一句——过程要看才展开。并行派出的几趟并排:`sm` 起两列网格,`sm` 以下竖排。派单那一次工具调用仍在它上面的工具组里(动词「派子代理」+ 派出去的那几句任务),回合结束后那一组收成一行,卡片是留下来的那一样。
- **提问轮次是对话流里的一张选择卡片**(CONTEXT.md 提问轮次):材质与 agent 的回复卡逐字相同(`rounded-lg border border-overlay-line bg-surface px-4 py-3`,不带阴影)——它是 agent 说的一段话,只是这一段要人回答,不另起一种容器。卡头一行 `text-sm text-text-muted`:QuestionMarkCircled 图标 + 「提问轮次 · N 题」,已答与过期各挂一颗灰 Badge。题按 `<ol>` 排,每题「编号. 标题」是 `text-md font-medium`,多选在标题后缀一枚 `text-sm` 的「多选」,题干 `text-md text-text-secondary`——**标题不得小于它要回答的那句话**,轻重由字重分。选项单选走 Themes `RadioGroup`、多选走 `CheckboxGroup`(第 8.3 节的选择语义),推荐项挂一枚蓝 Badge「推荐」,**不替人先选上**——这一轮是人的裁决,预选会让整轮点一下「提交」就过去,哪几格是他答的分不清;每题末尾固定多一项「其他」,选中即在它下面现一格 `TextField`(`ml-6` 与选项文字对齐)。卡底一行左说明右一颗实心「提交这一轮」,整轮答完才可按。已答的卡片把控件换成所选答案逐行列出(CheckCircled + `text-primary`),过期的卡片只读地列出选项并在卡底写一句「这一轮被后来的消息顶掉了」。看得到会话但发不出消息的人看到的是过期那一档的只读形态。
- 输入区是一个框:随内容长高的原生 `<textarea>` 在上,下沿一排图标键(图片、在跑时的排队 / 插话切换、停止、发送)。带图时附件排在框顶、textarea 之上:56px 圆角缩略图(`border-card-line`),移除键是嵌在图内右上角的 20px 深色圆键,细指针下悬停才现、粗指针常显。选图或粘贴的那一刻就用本地预览占位,压淡加 Spinner 表示在传,传完才出现移除键;有图在传时发送键禁用。原生 textarea 而不是 Themes TextArea,因为它要嵌在框里与下沿那一排共用一道边(与 `ui/command` 的输入同理);框的边 `border-input`、底 `bg-surface`、阴影 `shadow-control`、焦点环 `--v8-shadow-focus` 都走令牌。回车发送、Shift+回车换行,输入法选词不发。**发送键的禁用态是全站唯一一处覆写 Themes 组件状态的地方**(`disabled:bg-accent-tint disabled:text-text-muted`,第 14 节那条禁令的例外):Themes 给实心键的禁用态是 12% 灰底加 `--v8-text-faint` 的图标,压在白底的输入框里几乎看不出还有这么一颗键,草稿空着的会话于是像没有发送入口。输入区自己就是这一页的底,页面底部留白收到 16px,不留 `PageBody` 默认那段给最后一张卡的空。
- `lg` 以下左栏不显示(切会话走头部起始那颗回产品的箭头键);`xl` 以下右栏不显示,头部「对话 / spec 与票」切换把右栏那一份换进中栏(`sm` 以下这颗切换单占标题下面一行,尺寸随评审记录页那一版取 `{ initial: "3", sm: "1" }`);这一场一条 spec 与票都没写下时右栏与这颗切换都不出现。
...
- 全屏预览是浮层,内容区 `calc(100vw-2rem)` × `calc(100dvh-2rem)`,材质与圆角照第 2.3 节的浮层令牌。顶部是图的标题(没有就「图表预览」)、`+` / `−` / 适应 / 1:1 四个键、当前缩放百分比与关闭;这一行可折行,`sm` 以下标题独占第一行、按键整排落到第二行——与按键挤在一行时标题只剩两个字宽。画布滚轮以光标为锚缩放(0.25×–8×,页面不跟着滚)、一指按住拖动平移(拖动时 `cursor-grabbing`)、**两指捏合以两指中点为锚缩放**,Esc 关闭。缩放只用 CSS `transform`,不引缩放库。「适应」分两档:宽屏两轴一起装进画布并居中;**窄屏(画布窄于 640px)只按宽度装**,高出来的部分留给纵向拖动——390px 上一张横着长的时序图两轴都装会落到 25%,节点里的字认不出来。
...
- **两栏选择器的固定高度只在 `sm` 及以上成立。**`ModelComposer` 在 `sm` 起是 `240px minmax(0,1fr)` 两栏、整体 380px 高，两列各自局部滚动。`sm` 以下这套高度作废：服务列与模型列按自身内容堆叠、不设高度、不开内部滚动，页面自己滚。390px 宽下 380px 的容器要塞两段列表，服务列只剩 93px 可视，列底那条「配置或修复服务」会压住半行服务——高度守住了，内容却看不全。列底那条辅助链接因此排在最后一项之后，同时保持 44px 命中高度。
...
面板内是文件列表加逐文件 diff：每个文件一张卡，卡头写路径、新增/修改/删除、`+N −M`（增走 `--v8-success`、删走 `--v8-danger`）与这个文件的发现数，点卡头展开或收起。diff 用 `font-mono text-xs` 的三列表格——旧行号、新行号、正文，新增行的行号槽 `--v8-success-tint`、代码格同色减半，删除行同理走 `--v8-danger-tint`；整个文件全是新增行时代码格不铺底色，只留行号槽；`sm` 以下收掉旧行号列，hunk 头走 `--v8-surface-sunken`(`@@ … @@` 范围次级色、其后的所在函数或类主文字色,上下各一道 `border-overlay-line`)。Finding 卡片插在它所指的那一行下面（侧滑点进来的那一条整张摊开，它锚定的那一行新行号格走 `bg-accent-track` 加主色半粗数字；同文件的其它 Finding 收成「徽章 + 单行标题」的一行，点开才是整张卡）：那一格铺 `--v8-surface-sunken`、换回 `font-sans text-base`，卡片是内嵌其上的一张圆角卡（`border-overlay-line` + `shadow-control`，侧滑点进来的那一条描 `border-primary` 加 `bg-accent-tint`）——表格的等宽字体不传进卡片。卡里依次是徽章与标题、正文（主文字色，「影响」「建议」标签次级色，行宽封 56rem，反引号段按行内代码）、一列标签对齐的元信息（报出模型 chip、行作者、延续、处置、备注；56px 定宽标签次级色、值主文字色）、处置行（行尾是 Forge 外链）；容器宽到 64rem 时元信息与处置行挪到右栏。文件位置一律走 `FilePath`：目录次级色、文件名主文字色半粗。已处置项标题划线加绿勾、正文退成次级色（不整段划线），未处置项带优先级徽章；模型失败原因整段摊在最上面的红色 Callout 里，不折叠。

阶段详情使用同一浮层规格承载单条 Finding 的代码差异与单轮审查轨迹。Finding 侧滑头部右侧是「n / N」计数与上一条 / 下一条两颗 soft 灰 `IconButton`(`J` / `K` 同效),顺序跟列表当前筛选走。diff 代码按文件扩展名做语法高亮,用 GitHub 亮色主题(Primer)的七色,收成 `--v8-code-*` 令牌——关键字 #cf222e、函数与类名 #6639ba、常量与数字 #0550ae、字符串 #0a3069、内建 #953800、标签名 #116329、注释 #59636e,每一种在白底与增删行 tint 上都过 4.5:1;认不出语言的文件显示纯文本。Finding 列表入口使用 `FileTextIcon` 图标加「代码差异」字样(`sm` 以下只留图标)与动态可访问名称,Review Run 入口使用 `ReaderIcon` 加「审查轨迹」;界面不使用「看这处」「看这一轮」等依赖视觉上下文的动作名称。diff 的旧 / 新行号列固定,代码列按可用宽度 soft wrap，续段悬挂缩进、与代码列对齐；续行仍属于同一个逻辑行,不重复行号,缩进与空格保留,侧滑自身不产生横向滚动。移动端关闭、Forge 外链、处置和 Reviewer 展开动作的触控区域至少 44px。该 Primitive Dialog 的 Portal 固定挂到 `PanelTheme` 内的 `#panel-portal`,确保 Themes 组件继承同一套颜色、字号和圆角变量。
...
⌘K / Ctrl+K 是一级入口，顶栏常驻。快捷键监听挂在 window 上，任意页面任意焦点都能唤起。面板是 584px 宽、`--v8-radius-panel` 圆角、`--v8-palette-bg` + blur(50px)、`--v8-shadow-palette` 的 Spotlight 材质。当前只收导航跳转——这是面板里唯一一类「知道目标名字就想直接过去」的动作；触发评审、注册仓库这类动作都要先选对象，进来也只是又一次跳转。底部显示 ↑↓ / ↵ / esc 三条键位提示，触屏（`pointer: coarse`）上隐藏——那里没有方向键也没有 esc。

`sm` 以下面板顶靠视口：整宽、顶边不倒角、顶部留 `env(safe-area-inset-top)`，列表高度上限收到 `100dvh` 减去搜索行，滚动发生在列表内。关闭键（44px）在搜索行右端，窄屏或触屏出现，鼠标宽屏上让位给 esc 提示；关闭后焦点回到顶栏的 ⌘K 入口。桌面布局不变。
...
- `Markdown`：agent 回复的 GFM 渲染(react-markdown + remark-gfm,`skipHtml`),元素映射到产品排版令牌,表格套横向滚动、链接新标签打开。GFM 的任务清单换成静态图标:已勾 `CheckCircledIcon` 走 `text-success-icon`、未勾 `CircleIcon` 走 `text-text-faint`,状态给读屏留一句「已完成」/「未完成」,那一项去掉圆点——浏览器画的灰方框既不是这套设计的材质,又长得像一个点不动的控件。
...
- 页面不得用 Tailwind 覆盖 Themes 组件内部的 hover、focus、selected、disabled、loading 或 invalid 状态，也不得为同一语义另写一套 utility 外观。全站唯一的例外是会话输入区的发送键禁用态（7.5）：Themes 给实心键的禁用态在白底输入框里看不出还有这么一颗键。再要破例先改这一条，不在页面里自己开口子。
