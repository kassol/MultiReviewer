...
Agent 会话页的中栏是一块占满视口的聊天工作台,整页不滚:`PageBody` 加 `h-full`,壳只在这条路由上给 `main` 加 `min-h-0`(其余页面 main 随内容长高、整体在 `#panel-main-scroll` 里滚,加了会让 sticky 的移动端 Tab 栏跟着滚走),高度沿 flex 链给下来,不写视口常量。头部一行细标题(起始一颗回产品的箭头键、用途 `text-3xl` 标题、在跑徽章、一行时刻 / 建立人 / 用量、每仓库一行的基点),对话流自己滚,输入框钉在底部。产品名不在左栏里再重复一张小卡:左栏的产品列表已经把当前产品高亮出来了。**`sm` 以下头部收成一行**:箭头键只剩图标(`sm` 起键上带产品名),时刻 / 建立人 / 用量与基点收进标题右侧那颗 disclosure,摊开才看——这三行元信息在 390px 上把对话流压到 57% 的屏。新条目来时人在底部(距底 ≤ 80px)就跟着滚,翻上去看旧消息时不打扰,右下角浮一颗 44px 高的「最新」胶囊送回底部;胶囊现身时对话流末尾多留一段,它因此不压最后一条消息。
