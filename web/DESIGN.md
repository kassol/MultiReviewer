...
**OWN-WORLD** 灰底浮白卡：页面底 `--v8-bg` #f5f6f8，卡面 `--v8-surface` 纯白，卡靠 1px 发丝边 `--v8-border-card` rgba(0,0,0,0.055) 加漫射阴影 `--v8-shadow-card` `0 1px 6px rgba(0,0,0,0.04)` 浮起来。顶栏、移动端 Tab 栏、命令面板与运行详情面板是半透明白 + `backdrop-filter` 毛玻璃，层级越高模糊半径越大、底色越不透明。Apple 蓝 `--v8-accent` #0071e3 是唯一主色，动作、选中、链接、图表共用它。绿 `--v8-success` #1a7f37、琥珀 `--v8-warning-icon` #bf8700、红 `--v8-danger` #cf222e 只承载状态，一律以 soft tint 胶囊或 octicon 式图标出现，不用高饱和大色块。

**STORY** 打开面板先落在总览：KPI 四卡回答“今天跑了多少、有多少待处置、七日处置率、模型健不健康”，右栏回答“哪个模型服务出了问题”。发现异常沿顶栏导航进入评审记录，选中运行行右侧浮出详情面板，按模型逐条读发现与失败原因，就地重跑。配置类工作（模型服务、审查策略、仓库、访问控制）都是主从结构：左边选对象，右边改它。⌘K 在任何页面任何焦点位置都能唤起，直接跳到目标页。

**FIRST VIEWPORT** 桌面端顶部是双层毛玻璃顶栏：上层品牌方块、面包屑、300px 宽 ⌘K 搜索入口与头像菜单；下层 underline 导航，当前项字重 650 加 3px 蓝色圆头指示条。顶栏之下整屏都是内容：总览是 KPI 四卡加双列，列表页是工具行加长列表卡，主从页是 264px 侧栏加详情。390px 下导航层收起，改成底部毛玻璃 Tab 栏；顶栏只留一行（品牌 + 搜索图标 + 头像），页面主体不变。
...
面板默认进入总览。首屏优先展示当日运行量、待处置发现、七日处置率与模型服务健康。无 `review:read` 权限的账号按导航顺序落到自己的第一个可见页。
...
| 警告图标 #bf8700、文字 #9a6700 | 保留双色对 | 亮琥珀当文字在白底上过不了 AA |
...
- **导航计数徽章只做仓库数**。`/repos` 返回完整数组，长度就是总数；`/runs` 是游标分页，没有总数端点，不拿第一页条数冒充总数。模型服务的告警点取 `/setup-status` 的 `hasRunnableModelService`。
...
| --- | --- | --- | --- | --- |
| 成功 | `--v8-success` #1a7f37 | 同左 | `--v8-success-tint` rgba(26,127,55,0.1) | 已验证、已完成、正常、可运行 |
| 警告 | `--v8-warning-icon` #bf8700 | `--v8-warning` #9a6700 | `--v8-warning-tint` rgba(191,135,0,0.1) | 信息缺失、需要关注、降级运行 |
| 错误 | `--v8-danger` #cf222e | 同左 | `--v8-danger-tint` rgba(207,34,46,0.09) | 失败、不可运行、删除、无效输入 |
| 信息 | `--v8-text-muted` #86868b | 同左 | `--v8-fill` | 中性提示、未知、尚未开始 |

警告是唯一的双色对：图标与状态点用亮的 #bf8700 保持辨识度，文字压到 #9a6700 才在白底上过 AA。成功与危险的图标、文字同色。
...
- `lg=1024px` 是仓库与模型服务主从双栏的切换点，网格是 `264px minmax(0,1fr)`。外壳仍在 `sm=640px` 切换；640–1023px 内容区继续使用列表／详情单层布局。
- 仓库页双栏下左右各自局部滚动；模型服务页整页跟着外壳的 `panel-main-scroll` 一起滚，列表与详情不各开滚动区。
...
外壳下挂七页：`/` 总览、`/runs` 评审记录、`/repos` 仓库、`/stats` 处置率、`/credentials` 模型服务、`/settings` 审查策略、`/access` 访问控制，另有 `/password` 与外壳外的 `/login`。导航顺序即 `NAV` 数组顺序，总览打头、账户项收尾。
...
交付前在总览、评审记录、模型服务三个代表页面验证：

1. 顶栏毛玻璃：滚动内容穿过顶栏时背景确实虚化，`--v8-chrome-bg` 下的文字与图标对比度仍达标；顶栏底边可见但不抢眼。
2. 导航指示条：激活项 3px 圆头蓝条左右各内缩 12px；激活与未激活切换时文字基线不上下跳；计数徽章在激活时转蓝、未激活时为灰；模型服务异常时琥珀点出现并有可访问名称。
