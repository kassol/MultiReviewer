...
左栏在两页的滚动锚点不同、视觉位置相同:会话页整屏不滚,左栏 `lg:h-full` 自己滚;产品页整页在 `#panel-main-scroll` 里滚,左栏 `lg:sticky lg:top-[88px]`(顶栏高度)加 `lg:max-h-[calc(100vh-112px)]` 自己滚,并 `lg:self-start` 让它不被 flex 拉满——否则 sticky 不生效。`lg` 以下两页都竖排:产品页依次是产品列表、仓库、我的会话、概览、产品知识;会话页左栏整块不显示,头部那行产品名链接是回产品的入口。
