...
- **产品 tracker** 的计数带单位(「N 条 spec · M 张票」),不写成「N / M」——后者读着像进度。spec 行整行 hover 铺一层 `bg-sunken`:标题在左、「导出」在右端,宽屏上两头隔着几百像素,没有底色时看不出它们属于同一条;标题那颗 ghost 键静息态带一颗 chevron,hover 与 focus 下划线,否则它与纯文字无异。**打开的是哪一条 spec 记在地址的 `?spec=` 上**,与阶段详情的 `?finding=` 同一写法(`useRouterState` 读,开关都走 `replace`):会话页右栏「本会话写的 spec 与票」那几行链过来时带的就是它,产品页落地即展开那条的全文弹窗并滚到 tracker 区(`scroll-mt-[88px]` 让开顶栏),刷新与分享链接打开的是同一条。票那几行不带这个参数——读会话那一份只给票的 id 与标题,它属于哪条 spec 服务端没回。
