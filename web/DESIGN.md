...
- 消息两种形态:人的消息是右对齐的 `bg-accent-tint` 气泡(`rounded-2xl rounded-br-md`,`max-w-[80%]`),agent 的一条完整回复是一张占满列宽的内嵌卡(`border-overlay-line` + `shadow-control`,与详情内嵌卡同一材质),时刻与「展开 / 收起」「阅读」「复制 Markdown」放在一行 footer 里:`md` 以下与折叠态在卡底(折叠态的「展开」常显),`md` 起非折叠态这一行浮在卡片右上沿(发丝边小条、`shadow-control`,指到卡片才显,不占卡内高度——一句话的回复不再空着半张卡;`sm` 以下这几颗只剩图标、文字留给读屏——带文字要 234px,卡里只有 226px,末一颗会被切掉;卡不带阴影,发丝边在灰底上足够);超过 800 字或 12 个换行的长回复默认收到 320px 高、底部渐隐,「阅读」开 1200px 宽的阅读视图(`Markdown` 的 `article` 档:正文 14px / 1.7、限宽 120ch)。时刻在 footer 左侧,`md` 以下常显、`md` 起指到才显(改 opacity,布局不动)。
