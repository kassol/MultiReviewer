...
- 消息两种形态:人的消息是右对齐的 `bg-accent-tint` 气泡(`rounded-2xl rounded-br-md`,`max-w-[80%]`),agent 的一条完整回复**不套卡**,正文直接落在对话列(页面底)上——一场会话几十条回复,每条一张白卡就是一面卡片墙,需要容器的是人的气泡、工具行、子代理卡与提问卡;流式那一版同样没有卡,落库后不跳。时刻与「展开 / 收起」「阅读」「复制 Markdown」放在一行 footer 里:`md` 以下与折叠态在卡底(折叠态的「展开」常显),`md` 起非折叠态这一行浮在这条回复的右上沿(发丝边小条、`shadow-control`,指到卡片才显,不占回复自己的高度;`sm` 以下这几颗只剩图标、文字留给读屏——带文字要 234px,卡里只有 226px,末一颗会被切掉;折叠态底部的渐隐收到页面底 `from-background`);超过 800 字或 12 个换行的长回复默认收到 320px 高、底部渐隐,「阅读」开 1200px 宽的阅读视图(`Markdown` 的 `article` 档:正文 14px / 1.7、限宽 120ch)。时刻在 footer 左侧,`md` 以下常显、`md` 起指到才显(改 opacity,布局不动)。
