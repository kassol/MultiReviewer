...
- **产品知识**分三段(术语表、仓库关系、产品决策)。陈述段落与 Markdown 正文同一档字号(`text-lg`)、行宽封在 `max-w-[46em]`——它们常常是整段话,铺满不设上限的内容轨之后眼睛要横跨整屏才回到行首;徽章行与出处行不限宽,它们是扫的,不是读的。术语表按主题分组,每组一个默认展开的 Collapsible 段头(组名 + 条数,没分组的那几条挂在「其余」下),组与组之间比组内两条之间松一档。产品决策每条左侧一根 2px 状态条:生效走 `border-l-success-icon`,被取代走 `border-l-neutral-dot`——一列决策里哪几条还算数,扫一眼边缘就看得出,不必逐条读徽章。
