...
2026-08-24 已在部署实例完成验收：2056px 验证双栏、独立滚动、40 行增量模型列表、统一选中态和路由分块；768px 验证模型服务与仓库的单层列表／详情及返回入口；390px 验证顶部横向导航、移动端记录与统计、双月纵向日期范围、44px 日期触控目标和弹窗焦点返回。三个宽度均无页面横向溢出。
