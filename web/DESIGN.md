...
- **提问轮次是对话流里的一张选择卡片**(CONTEXT.md 提问轮次):材质与 agent 的回复卡逐字相同(`rounded-lg border border-overlay-line bg-surface px-4 py-3`,不带阴影)——它是 agent 说的一段话,只是这一段要人回答,不另起一种容器。卡头一行 `text-sm text-text-muted`:QuestionMarkCircled 图标 + 「提问轮次 · N 题」,已答与过期各挂一颗灰 Badge。题按 `<ol>` 排,每题「编号. 标题」是 `text-base font-medium`,多选在标题后缀一枚 `text-sm` 的「多选」,题干 `text-md text-text-secondary`。选项单选走 Themes `RadioGroup`、多选走 `CheckboxGroup`(第 8.3 节的选择语义),推荐项挂一枚蓝 Badge「推荐」,**不替人先选上**——这一轮是人的裁决,预选会让整轮点一下「提交」就过去,哪几格是他答的分不清;每题末尾固定多一项「其他」,选中即在它下面现一格 `TextField`(`ml-6` 与选项文字对齐)。卡底一行左说明右一颗实心「提交这一轮」,整轮答完才可按。已答的卡片把控件换成所选答案逐行列出(CheckCircled + `text-primary`),过期的卡片只读地列出选项并在卡底写一句「这一轮被后来的消息顶掉了」。看得到会话但发不出消息的人看到的是过期那一档的只读形态。
