...
- 阶段详情的 Finding 列表按滚动逐段渲染:首段 50 项,接近底部时追加一段,一张同根因组卡算一项(组内成员随组卡一起画)。尾部一行「已显示 N / M 条，向下滚动继续加载」,全部画完这一行就不出现。筛选与顶部三个计数仍对全量算;换筛选条件回到首段;列表第一张卡已经滚过吸顶工具条下沿时,只把它滚回贴着那条线,还没滚过就不动(issue #444)——滚回页顶那一下工具条先脱开再落回,整页跳一截。几百条的阶段一次画全要两秒、留下两万多个 DOM 节点,而人一屏看得到的只有头几条。
