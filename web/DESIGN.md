...
- 消息两种形态:人的消息是右对齐的 `bg-accent-tint` 气泡(`rounded-2xl rounded-br-md`,`max-w-[80%]`),agent 的一条完整回复是一张占满列宽的内嵌卡(`border-overlay-line` + `shadow-control`,与详情内嵌卡同一材质),卡底一行 footer 放时刻与「展开 / 收起」「阅读」「复制 Markdown」;超过 800 字或 12 个换行的长回复默认收到 320px 高、底部渐隐,「阅读」开 880px 宽的阅读视图(`Markdown` 的 `article` 档:正文 14px / 1.7、限宽 72ch)。时刻在 footer 左侧,`md` 以下常显、`md` 起指到才显(改 opacity,布局不动)。
