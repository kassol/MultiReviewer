...
export function ModelComposer({ value, onChange, provider, onValidityChange }: ModelComposerProps) {
  ...
  const toggle = (identity: string): void => {
    ...
    // 新选进来的模型带它自己的第一档,与辅助模型控件同一处规则。
    const first = candidateByIdentity.get(identity)?.runtime.thinkingLevels[0] ?? "off";
    onChange([...value, modelRefWithLevel(identity, first)]);
  };
  ...
  const setLevel = (identity: string, level: ThinkingLevel): void => {
    onChange(
      value.map((item) => (item.identity === identity ? modelRefWithLevel(identity, level) : item)),
    );
  };
                ...
                >
                  <div className="min-w-0 flex-1 space-y-1.5">
                    ...
                    {candidate === undefined ? null : (
                      <ThinkingLevelPicker
                        levels={candidate.runtime.thinkingLevels}
                        value={thinkingLevel ?? "off"}
                        onChange={(next) => setLevel(identity, next)}
                        identity={identity}
                        size={{ initial: "2", sm: "1" }}
                        ariaLabel={`${identity} 的思考档位`}
                        placeholder="选一档"
                      />
                    )}
                  </div>
