...
/** 建会话与重梳接口收的那一行基点(issue #352)。`branch` 是他选它时浏览的那条分支。 */
...
/**
 * 行组里人动过的那几行,换成建会话与重梳接口收的那一份(issue #352)。分支名只在他是从分支
 * 里选的时候带上——从 Tag 里选的那一个不属于任何分支,服务端按生效的默认分支记。
 */
export function pickedBaselines(
  ...
  return Object.entries(picked).map(([key, selection]) => {
    ...
    return {
      ...
      repo,
      sha: selection.sha,
      ...(selection.source?.kind === "branch" ? { branch: selection.source.name } : {}),
    };
  });
}
