  ...
  return (
    <>
      ...
      <div className="flex max-w-[1100px] flex-col gap-6 p-5">
        {feedback === null ? null : <p role={feedback.error ? "alert" : "status"} className={feedback.error ? "text-destructive" : "text-muted-foreground"}>{feedback.text}</p>}
        {loadError === null ? null : <p role="alert" className="text-destructive">{(loadError as Error).message}</p>}
        {pending ? <><Skeleton className="h-56" /><Skeleton className="h-80" /></> : (
          <>
            <section className="flex flex-col gap-2" aria-labelledby="users-heading">
              <h2 id="users-heading" className="text-base font-semibold">用户 <span className="font-mono text-muted-foreground">{users.length}</span></h2>
              <div className="overflow-x-auto rounded-md border border-border">
                <table className="w-full min-w-[860px] border-collapse text-left">
                  <thead className="bg-muted/60 text-xs text-muted-foreground">
                    <tr><th className="px-3 py-2 font-medium">用户名</th><th className="px-3 py-2 font-medium">显示名</th><th className="px-3 py-2 font-medium">角色</th><th className="px-3 py-2 font-medium">创建</th><th className="px-3 py-2 font-medium">最后登录</th><th className="px-3 py-2"><span className="sr-only">操作</span></th></tr>
                  </thead>
                  <tbody>
                    {users.map((user) => (
                      <tr key={user.username} className="border-t border-border">
                        <td className="px-3 py-2 font-medium"><div className="flex items-center gap-1.5">{user.username}{user.isSystemAdmin ? <Badge>系统管理员</Badge> : null}{user.mustChangePassword ? <Badge className="bg-warning/10 text-warning">待改密</Badge> : null}</div></td>
            ...
            <section className="flex flex-col gap-2" aria-labelledby="permissions-heading">
              <h2 id="permissions-heading" className="text-base font-semibold">权限</h2>
              <div className="flex items-start gap-2 rounded-md border border-border bg-muted/40 px-3 py-2"><ShieldCheck className="mt-0.5 size-4 shrink-0" /><p className="text-muted-foreground"><span className="font-medium text-foreground">系统管理员</span>不在矩阵里：权限全开、不可编辑，而且可以有多个（<span className="font-mono">{adminCount}</span> 人）。下面这些格子只管自定义角色。</p></div>
              {unclaimed.size === 0 || roles.length === 0 ? null : <div className="rounded-md border border-warning/40 bg-warning/10 px-3 py-2 text-warning">有 <span className="font-mono">{unclaimed.size}</span> 格权限从未被任何角色勾过。除系统管理员外，没人能使用它盖住的功能。</div>}
              {roles.length === 0 ? (
                <Card className="items-start gap-2 px-4 py-5"><p className="font-medium">还没有任何角色，所以矩阵还没有一列</p><p className="text-muted-foreground">角色不预置。新建角色会从全空开始；在那之前，除系统管理员外没人能使用任何页面。</p><Button className="mt-1" onClick={() => setCreateKind("role")}><Plus />新建角色</Button></Card>
              ) : (
                <div className="overflow-x-auto rounded-md border border-border">
                  <table className="w-full border-collapse text-left">
                    <thead><tr className="bg-muted/60 text-xs text-muted-foreground"><th className="sticky left-0 z-10 min-w-72 bg-muted px-3 py-2 font-medium">权限格</th>{roles.map((role) => <th key={role.id} className="min-w-32 border-l border-border px-3 py-2 text-center font-medium whitespace-nowrap"><span>{role.name}</span><span className="block font-normal"><span className="font-mono">{users.filter((user) => user.roleId === role.id).length}</span> 人</span><Button variant="ghost" size="xs" className="mt-1 text-destructive" onClick={() => setConfirm({ kind: "delete-role", id: String(role.id), label: role.name })}><Trash2 />删除</Button></th>)}</tr></thead>
                    <tbody>{RESOURCES.flatMap((resource) => [
                      <tr key={`${resource}-heading`} className="border-t border-border bg-muted/30"><td colSpan={roles.length + 1} className="sticky left-0 px-3 py-1 text-xs font-medium text-muted-foreground">{resource}</td></tr>,
                      ...PERMISSION_INFO.filter((permission) => permission.resource === resource).map((permission) => {
                        const missing = unclaimed.has(permission.id);
                        return <tr key={permission.id} className={`border-t border-border ${missing ? "bg-warning/10" : ""}`}><td className={`sticky left-0 z-10 px-3 py-2 ${missing ? "bg-[color-mix(in_oklab,var(--warning)_10%,var(--background))]" : "bg-background"}`}><div className="flex items-baseline gap-2"><span className="font-medium">{permission.action}</span><span className="font-mono text-xs text-muted-foreground">{permission.id}</span>{missing ? <span className="text-xs text-warning">未启用</span> : null}</div><p className="text-xs text-muted-foreground">{permission.hint}</p></td>{roles.map((role) => <td key={role.id} className="border-l border-border px-3 py-2 text-center"><input type="checkbox" aria-label={`${role.name} ${permission.id}`} checked={role.permissions.includes(permission.id)} disabled={updateRole.isPending} onChange={() => updateRole.mutate({ role, permission: permission.id })} className="size-4 accent-primary" /></td>)}</tr>;
                      }),
                    ])}</tbody>
                  </table>
                </div>
...
function CreateDialog({ kind, busy, onClose, onUser, onRole }: { kind: "user" | "role" | null; busy: boolean; onClose: () => void; onUser: (input: { username: string; displayName: string; password: string }) => void; onRole: (name: string) => void }) {
  ...
  return <Dialog open={kind !== null} onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent><form onSubmit={submit} className="contents"><DialogHeader><DialogTitle>{kind === "user" ? "建号" : "新建角色"}</DialogTitle><DialogDescription>{kind === "user" ? "新账号先没有角色，首次登录必须改密码。" : "角色从全空权限开始，建好后在矩阵里逐格勾选。"}</DialogDescription></DialogHeader>{kind === "user" ? <div className="flex flex-col gap-2"><Label htmlFor="new-username">用户名</Label><Input id="new-username" autoFocus value={username} onChange={(event) => setUsername(event.target.value)} /><Label htmlFor="new-display-name">显示名</Label><Input id="new-display-name" value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="可留空" /><Label htmlFor="new-password">临时密码</Label><Input id="new-password" type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} /></div> : <div className="flex flex-col gap-2"><Label htmlFor="new-role-name">角色名</Label><Input id="new-role-name" autoFocus value={name} onChange={(event) => setName(event.target.value)} /></div>}<DialogFooter><DialogClose asChild><Button type="button" variant="outline">取消</Button></DialogClose><Button type="submit" disabled={busy || (kind === "user" ? username === "" || password === "" : name === "")}>{busy ? "创建中…" : "创建"}</Button></DialogFooter></form></DialogContent></Dialog>;
}
