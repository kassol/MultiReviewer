  ...
  return (
    <dl className="grid gap-px overflow-hidden rounded-md border bg-border sm:grid-cols-2 lg:grid-cols-3">
      <div className="bg-background p-3">
        ...
        {service.target === undefined ? null : (
          <div className="mt-2 space-y-0.5 text-xs text-muted-foreground">
            <p className="break-all">地址：{service.target.baseUrl ?? "未提供"}</p>
            <p>接口协议：{service.target.api ?? "未提供"}</p>
          </div>
        )}
      </div>
      <div className="bg-background p-3">
        ...
        {canReadCredential ? (
          <div className="mt-2 space-y-0.5 text-xs text-muted-foreground">
            <p>
              尾 4 位：
              <span className="font-mono tabular-nums">{service.credential.last4 ?? "未提供"}</span>
            </p>
            <p>
              更新：
              <span className="font-mono tabular-nums">{localMinute(service.credential.updatedAt)}</span>
            </p>
            <p>
              验证：
              <span className="font-mono tabular-nums">{localMinute(service.credential.verifiedAt)}</span>
            </p>
            <p className="break-all">验证模型：{service.credential.validationModel ?? "未提供"}</p>
            <p>
              验证来源：
        ...
        <div className="bg-background p-3">
          ...
          <dd
            className={cn(
              ...
              (service.directory.state === "undiscovered" ||
                service.directory.state === "discovery-failed") &&
                "text-destructive",
            )}
          >
          ...
          <div className="mt-2 space-y-0.5 text-xs text-muted-foreground">
            <p>
              最近尝试：
              <span className="font-mono tabular-nums">{localMinute(service.directory.lastAttemptAt)}</span>
            </p>
            <p>
              最近成功：
              <span className="font-mono tabular-nums">{localMinute(service.directory.lastSuccessAt)}</span>
            </p>
...
function ReferenceBlockers({ references }: { references: ModelReference[] }) {
  ...
  return (
    <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2">
      <p className="font-medium text-destructive">仍在引用的位置</p>
      <ul className="mt-2 space-y-2">
        {references.map((reference) => (
          <li key={reference.identity}>
            <p className="break-all font-mono text-xs">{reference.identity}</p>
      ...
      <div className="space-y-4">
        ...
        {service.providerState === "name-conflict" ? (
          <div className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-destructive">
            ...
            <p className="mt-0.5 text-xs">配置仍被保留；改名重建或移除冲突后才能恢复模型可用性。</p>
          </div>
        ...
        {canReadModels && service.models !== undefined ? (
          ...
          <section className="rounded-md border px-4 py-5">
...
export function ModelServicesPage({
  ...
  return (
    <>
      <PageHeader
        title="模型服务"
        description="检查 provider、模型凭据与模型目录三条独立状态；配置 Pi 内置 provider，或原子创建与切换自定义服务。"
        actions={(
          <div className="flex flex-wrap items-center gap-2">
      ...
      {!canReadServices ? (
        <div className="max-w-[760px] p-5">
          <Card className="gap-2 px-4 py-5">
            ...
            <p className="text-muted-foreground">
              当前会话只能写模型凭据；可从页头搜索进入内置 provider 配置，现有服务、模型与凭据字段不会显示。
            </p>
          </Card>
        </div>
      ) : query.isPending ? (
        ...
        <div className="max-w-[760px] p-5">
          <Card className="gap-2 px-4 py-8 text-center">
            <h2 className="text-base font-semibold">还没有模型服务</h2>
            <p className="text-muted-foreground">
              这里仅列已配置或保留异常状态的服务；可用页头搜索{canWriteCredential ? "配置" : "查看"} Pi 内置 provider。
            </p>
          </Card>
        </div>
          ...
          <Card className="self-start gap-0 overflow-hidden p-0">
            <div className="border-b bg-muted px-3 py-2">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-medium">模型服务检查表</h2>
              </div>
              <p className="mt-0.5 text-xs text-muted-foreground">仅列已配置与保留异常状态</p>
            </div>
            <div className="divide-y">
              {services.map((service) => (
                <button
                  ...
                  className={cn(
                    "w-full px-3 py-3 text-left transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset",
                    service.provider === selected.provider && "bg-muted",
                  )}
                    ...
                    <div className="mt-1 flex flex-wrap justify-between gap-x-3 gap-y-0.5 text-xs text-muted-foreground">
                      ...
                      <span>
                        ...
                        <span className="font-mono tabular-nums">
                          {localMinute(service.directory.lastSuccessAt)}
                        </span>
                      </span>
                    </div>
                  )}
