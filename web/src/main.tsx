...
function Shell() {
  ...
  // 模型服务的模型 tab 只在 lg 起占满视口:清单在卡内滚。窄屏它上面的东西太多,仍随整页滚。
  // 槽位照留:同一个服务的另外两个 tab 是整页滚的,切 tab 时内容不该横跳一个槽宽。
  const fillsViewportLg = /^\/credentials\/[^/]+\/models$/.test(pathname);
      ...
      */}
      ...
      >
        <TopBar
          ...
          onLogout={logout}
        />
        <main className={fillsViewport ? "min-h-0 min-w-0 flex-1" : fillsViewportLg ? "min-w-0 flex-1 lg:min-h-0" : "min-w-0 flex-1"}>
