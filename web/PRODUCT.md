...
已定方向，分票实现中（ADR 0010，2026-08-20）：

- 模型服务配置与模型组合选择继续分属两个页面，创建成功后直接进入审查策略并定位新服务。
- 添加与修改模型服务共用独立的三步流程页。模型发现失败或缺少目标模型时允许手填 model id，最终以真实推理决定是否生效。
- 已实现：模型服务详情以能否运行为主状态，概览、维护与模型分成三条稳定地址；空且未引用的内置 provider 只留在添加入口。
...
- 可运行的六面：登录、仓库、评审记录、处置率、模型服务、全局设置；其中全局设置待按 ADR 0010 改名为审查策略。
