...
- `src/components/detail-panel.tsx` — 主从列表详情面板的唯一外壳(`DESIGN.md` 10.2)。桌面 `md=768px` 起四边留 14px 浮在列表右侧,窄屏是 86dvh 的底部抽屉;关闭按钮、头部结构、正文滚动边界与底部动作条的位置都在这里定,页面只给标题那一列、进度这类头部附加块、正文和动作条。桌面宽度固定 920px,装完整 diff 与阶段汇总(评审记录、范围审查都是);更窄的档位在范围审查详情改成阶段汇总后没有消费者,已去掉。点面板外面默认关闭,调用方可以接管这一下——评审记录用它换成列表里的另一轮,而不是先关再点。
...
- `src/repos.tsx` — 仓库页,左列表右详情。`lg=1024px` 起双栏，列表和详情各自局部滚动；更窄时只显示一层，进入详情后提供返回仓库列表入口。模型覆盖是「跟随全局 / 自定义」两态:点「跟随全局」直接清覆盖;点「自定义」在详情内挂与审查策略共用的 `ModelComposer`,以当前生效组合为初值。不可用的既有选择可移除但不得随覆盖再次保存;服务端仍作最终校验。编辑态并成单栏容纳 460px 高的两栏选择器,不套对话框或外层表单。审查配置就绪前注册按钮禁用并指向审查策略,状态失效时已开的注册框随即卸载。注册使用 Themes Dialog,移除确认使用 Themes AlertDialog,不用阻塞渲染线程的 `window.confirm`。详情里的「评审记录」区块与全局评审记录读同一个端点、同一种行(`GET /stages` 加一对仓库过滤,issue #174):最近 8 个审查阶段,行的形状与点开的详情面板都复用 `runs.tsx` 导出的那几件,同一个阶段在两处是同一条记录。这个区块的卡头除「输 PR 号重跑」外还有「发起范围审查」入口(`canCreate`,issue #177),仓库预填。
- `src/runs.tsx` — 评审记录页:阶段列表、状态与来源筛选、轮次详情面板的外壳。页头除总处置率外挂「发起范围审查」入口(`canCreate`,即 `review:create`,issue #177),表单是共用的 `range-review-launch.tsx`,全局入口不预填仓库。**列表的每一行是一个审查阶段**(issue #174,读 `GET /stages`):`owner/repo` 加名字(导出的 `stageLabel`:有标题显示标题,没有的显示 `#编号`——pull request 用它的 PR 号,范围审查用它自己的标识,容器 PR 的序号不露面)、来源标记(pull request / 范围审查)、最新一轮的时间、阶段汇总三个数(`StageCounts`:待处置 / 已处置 / 已修复)与状态徽章(`StageStatusBadge`:进行中 / 已结束)。**筛选与分页都在服务端**:状态与来源两个 `SegmentedControl` 各自写进地址(`?status=` / `?source=`,默认都是全部),翻页按响应里的 `nextOffset` 递进 `?offset=`,仍由 IntersectionObserver 滚到底自动取下一页。**点开一行打开的是这个阶段最新一轮**:行上只带 `latestRunId`,轮次本身由导出的 `RunDetailPanelById` 读 `GET /runs/{id}`(仓库页复用同一个组件,两处点开的是同一份东西);一轮都还没跑过的阶段点它不做任何事,而不是打开一个空面板。列表里只要还有没跑完的最新一轮(`latestRunFinishedAt` 为 null)就每 10 秒续查,跑完即停。面板外壳共用 `components/detail-panel.tsx`,正文整块交给 `run-diff.tsx` / `stage-summary.tsx` / `run-trace.tsx` 三者之一,自己只留头部(结论徽章、commit、触发来源、处置进度条)与底部那两颗按钮;桌面宽度 920px,装的是完整 diff,464px 里一行代码要折三四次。`RunItem` / `RunFinding` / `StageItem` 三个契约类型从这里导出,时间格式化走 `lib/time.ts`。**打开哪一行与筛选哪一片都记在地址里**(`?run=`、`?status=` 与 `?source=`):总览要能直接落到某一轮,链接要能指明列表的哪一片,浏览器后退键要能收起详情。开合详情进历史记录,筛选切换用 `replace`(否则点几下分段控件就把历史塞满)。详情面板是主从列表的详情,不是打断流程的对话框:点到列表里另一行时换成那一行最新的一轮,不先关再点;点在筛选控件上的那一下也接住(行与控件各自带 `data-run-id` 与 `data-filter-kind` / `data-filter-value`,面板按矩形命中它们)。**面板顶上是「本轮 diff / 阶段汇总 / 审查轨迹」分段控件**:本轮 diff 与审查轨迹(issue #171,正文见 `run-trace.tsx`)两条链路都有,**阶段汇总只给 PR 触发的那一轮**(issue #168);范围审查的轮次不给这个开关,它的阶段汇总在范围审查详情页上。地址里的 `?file=` 是阶段汇总跳过来时带的落点文件,换一行或收起面板时跟着清掉。模型失败原因挂在结论徽章的 Tooltip 上,徽章自己已经是警告样式,外面不再多挂一个同款图标。
