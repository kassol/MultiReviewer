...
- `src/range-reviews.tsx` — 范围审查页。列表 + 发起表单 + 详情三块。**打开哪一条记在地址里**(`?range=`):链接能指到具体一个阶段,浏览器后退键能收起详情;详情与评审记录页共用 `components/detail-panel.tsx`(里面装的是 Finding 卡片、筛选与行内处置),是浮在列表上的主从详情面板,不是打断流程的对话框。**详情的主视图是这个阶段的汇总**(`stage-summary.tsx`,issue #168):最新一轮没报出的问题不再藏在旧轮里。轮次降为时间线,仍挂在推出它的那个比较项下面,每轮只标本轮的新报出 / 折叠 / 已修复 / 已延续 / 漏复核。发起表单不在这一页里(issue #177):页头的入口就是共用的 `range-review-launch.tsx`,三处发起的是同一件事。列表行与详情头部的名字是发起时填的标题,升级前的旧记录显示 `#编号`,`owner/repo` 退到下面那行。列表里还有进行中的就每 10 秒续查,全都进终态即停。时间线上每轮的结论徽章仍直接复用 `runs.tsx` 的 `runStatus`,同一轮在两页显示成同一个词。
