...
- `src/runs.tsx` — 评审记录页。详情面板的模型卡片里逐条列出本轮 Finding:正文、严重度、类别、文件与行,加一枚跳到 Forge 看原评论的链接;有 `finding:dispose` 时行内 resolve / unresolve 并可填处置备注,处置成功让 `runs` / `repo-runs` / `range-review` 三份轮次查询一起失效——处置进度条与列表读的是同一批 finding 行,只改本地状态两个数字会对不上。已处置的正文划线加绿勾,未处置的带优先级徽章;「已改动」自动处置(ADR 0013)同样按已处置呈现,署名那一行写「代码已改动 · 自动处置」而不是人名——它的 `disposedBy` 为空。撤回入口照给:人撤回之后这一条就是人工处置,自动规则不再碰它。fallback(没有评论 id)不给处置动作,只写明为什么。轮次来源用一枚 Themes Badge 标出:两条链路的 `pullNumber` 都指向真实 PR(范围审查那条指的是容器 PR),不标的话一行「acme/widgets #101」看不出这是人开的 PR 还是本工具自建的容器。**打开哪一轮与筛选哪一片都记在地址里**(`?run=` 与 `?filter=`):总览要能直接落到某一轮,链接要能指明列表的哪一片,浏览器后退键要能收起详情。开合详情进历史记录,筛选切换用 `replace`(否则点几下分段控件就把历史塞满)。详情面板是主从列表的详情,不是打断流程的对话框:点到列表里另一轮时换成那一轮,不先关再点。列表里只要还有未结束的轮次就每 10 秒续查,全部结束即停。结论筛选使用 Themes `SegmentedControl`,只过滤已经加载的 Review Run；桌面使用固定列宽的 Themes `Table` 并把横向滚动限制在表格内，窄视口保持按日期分组的单列记录；模型失败原因挂在结论徽章的 Tooltip 上,徽章自己已经是警告样式,外面不再多挂一个同款图标。
