...
- `src/components/detail-panel.tsx` — 主从列表详情面板的唯一外壳(`DESIGN.md` 10.2)。桌面 `md=768px` 起四边留 14px 浮在列表右侧,窄屏是 86dvh 的底部抽屉;关闭按钮、头部结构、正文滚动边界与底部动作条的位置都在这里定,页面只给标题那一列、进度这类头部附加块、正文和动作条。宽度是 `wide` / `narrow` 两档:wide 920px 装完整 diff 与阶段汇总(评审记录、范围审查),narrow 464px 留给只有元信息的详情。点面板外面默认关闭,调用方可以接管这一下——评审记录用它换成列表里的另一轮,而不是先关再点。
...
- `src/range-reviews.tsx` — 范围审查页。列表 + 发起表单 + 详情三块。**打开哪一条记在地址里**(`?range=`):链接能指到具体一个阶段,浏览器后退键能收起详情;详情与评审记录页共用 `components/detail-panel.tsx`(wide 一档:里面装的是 Finding 卡片、筛选与行内处置,464px 里每张卡片都要折成好几行),是浮在列表上的主从详情面板,不是打断流程的对话框。**详情的主视图是这个阶段的汇总**(`stage-summary.tsx`,issue #168):最新一轮没报出的问题不再藏在旧轮里。轮次降为时间线,仍挂在推出它的那个比较项下面,每轮只标本轮的新报出 / 折叠 / 已修复 / 已延续 / 漏复核。发起表单是 Themes Dialog:仓库用 `Select` 从 `/repos` 选,base 与比较项是两个 `TextField`。**同一 base 已有进行中那一档不当错误提示**——服务端 409 带 `needsConfirmation` 时把提交按钮换成「仍然发起」,再点一次带 `confirm: true` 重发。列表里还有进行中的就每 10 秒续查,全都进终态即停。时间线上每轮的结论徽章仍直接复用 `runs.tsx` 的 `runStatus`,同一轮在两页显示成同一个词。
- `src/stats.tsx` — 处置率页。日期范围只调用受控 `DateRangePicker`；桌面模型 × 分类矩阵使用 Themes `Table` 与粘性模型列，窄屏逐模型详情使用 Collapsible，保持 Finding Identity 统计口径、范围选择与移动端可访问性。分子是人工与自动之和(`disposed`),分母加上自动那一列后口径不变(`denominator`);「已延续」在服务端就退出了统计,前端不必也不该再补一档;人工 / 自动的拆分列在「按模型统计」那一段,矩阵里仍是一个合计比率——每格再拆两列会让矩阵没法读。只有 `Cell` 类型导出供总览页复用;总览页的 `denominator` 是自己复制的一行,避免把日期选择器那一整块拽进首屏分块。
...
- `src/lib/time.ts` — 本地时区时间格式化的唯一实现:`localDay`(年-月-日)、`localClock`(时:分)与拼起来的 `localMinute`。评审记录、范围审查、访问控制与 diff 视图共用,不各带一份 `padStart`。
