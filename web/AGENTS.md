...
- `src/range-reviews.tsx` — 范围审查页。列表 + 发起表单 + 详情三块。**打开哪一条记在地址里**(`?range=`):链接能指到具体一个阶段,浏览器后退键能收起详情;详情与评审记录页共用 `components/detail-panel.tsx`(里面装的是 Finding 卡片、筛选与行内处置),是浮在列表上的主从详情面板,不是打断流程的对话框。**详情的主视图是这个阶段的汇总**(`stage-summary.tsx`,issue #168):最新一轮没报出的问题不再藏在旧轮里。轮次降为时间线,仍挂在推出它的那个比较项下面,每轮只标本轮的新报出 / 折叠 / 已修复 / 已延续 / 漏复核。发起表单是 Themes Dialog:仓库用 `Select` 从 `/repos` 选,base 与比较项是两个 `TextField`。**同一 base 已有进行中那一档不当错误提示**——服务端 409 带 `needsConfirmation` 时把提交按钮换成「仍然发起」,再点一次带 `confirm: true` 重发。列表里还有进行中的就每 10 秒续查,全都进终态即停。时间线上每轮的结论徽章仍直接复用 `runs.tsx` 的 `runStatus`,同一轮在两页显示成同一个词。
