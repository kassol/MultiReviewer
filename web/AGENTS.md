...
- `src/components/detail-panel.tsx` — 主从列表详情面板的唯一外壳(`DESIGN.md` 10.2)。桌面 `md=768px` 起四边留 14px 浮在列表右侧,窄屏是 86dvh 的底部抽屉;关闭按钮、头部结构、正文滚动边界与底部动作条的位置都在这里定,页面只给标题那一列、进度这类头部附加块、正文和动作条。宽度是 `wide` / `narrow` 两档:wide 920px 装完整 diff(评审记录),narrow 464px 装元信息与轮次列表(范围审查)。点面板外面默认关闭,调用方可以接管这一下——评审记录用它换成列表里的另一轮,而不是先关再点。
