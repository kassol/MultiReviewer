...
- `src/range-review-actions.tsx` — 一个范围审查阶段的两个动作(issue #176):`AdvanceAction` 推进比较项(对话框里装 `commit-picker.tsx` 的 `baseLocked` 那一档,issue #179:base 与当前比较项、新选的比较项都只以短 sha 显示,手输框已删,非 base 后代的行置灰)与 `CompleteAction` 标记审查完成(AlertDialog 二次确认,写明容器 PR 会关、分支会删、未处置的 Finding 怎么算)。阶段详情页与范围审查页共用它,`disabled` 那一格给已经审查完成的阶段用。`RangeReview` 契约类型与 `RANGE_REVIEWS_QUERY_KEY` 也从这里导出;动作成功后按 `range-reviews` / `range-review` / `stage-detail` / `stage-summary` 四段整片失效,两个页面回来时看到的都是新状态。
