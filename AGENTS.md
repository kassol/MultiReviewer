...
- 2026-08-25: 修「延续」的两处口径(issue #167 的 spec 评审)。**人把处置改回未处置之后,延续也不再碰它**:此前只看处置值,人撤回处置的那条下一轮照样被交接到新位置、旧评论照样被 resolve,与「人显式处置过就归人管」这条规则相反;判据现在与「已修复」自动处置完全一致。**承接的新位置必须落在本轮 diff 之内**:落在 diff 之外的那条只能写进 review 正文、没有可 resolve 的载体,承接它会让旧评论被关掉而新位置接不住,处置率的分母还会凭空少一条;这种情况现在不承接,旧行留在未处置等人。细节见 `src/AGENTS.md`。
