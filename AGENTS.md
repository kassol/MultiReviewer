...
- 2026-09-04: 落地 issue #247。**服务重启后中断的 Review Run 不再永远显示「进行中」**:启动时把所有没有结束时间的轮次改判失败,结束时间取启动时刻,失败原因是「服务重启,上一轮没跑完」,并按这一轮已固定的 Reviewer 逐个落在轮次详情的模型行上——与其他失败一轮同一种看法。被改判的轮次在 PR 上残留的 👀 随即撤掉,撤不动只记一行日志,不阻塞启动也不影响改判。面板从此不再对这些轮次轮询,轨迹页照实停在崩溃前最后一条事件,逐轮「重新运行」入口对它可用(重跑从头开始,本工具不续跑已完成批次)。没有中断轮次时启动路径零改动,一次 Forge 都不问。工作副本准备(issue #184)与基点探索(issue #205)早有同样的规则,这一票把 Review Run 补齐。细节见 `src/AGENTS.md`。
