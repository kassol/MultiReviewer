...
- `src/` — 编排服务源码,结构约定见 `src/AGENTS.md`。进程入口是 `src/main.ts`;`src/recover-finding-content.ts` 是恢复已有评审记录里缺失的影响与修改建议的一次性操作入口(issue #268),随源码进镜像,在容器里跑。
...
- `pnpm test` — 仅测试
- `node src/recover-finding-content.ts --repo <owner>/<仓库>` — 恢复已有评审记录里缺失的影响与修改建议(issue #268)。默认只预览、不写库:只读打开库,列出可补回的条数与来源、跳过的条数与原因;加 `--apply` 才写,写之前先 `VACUUM INTO` 在库旁留一份 `<库>.bak-<时刻>` 快照,再在一个事务里只补 NULL 的两列与延续承接的历史说法,重复执行没有第二份副作用。范围三选一:`--repo <owner>/<仓库>`、`--run <轮次 id>`、`--all`;库路径读 `--db`,不给读 `MULTIREVIEWER_DB`。配了 `MULTIREVIEWER_GITEA_URL` 与 `MULTIREVIEWER_GITEA_TOKEN` 时,轨迹里找不到的会去读那一行自己发出去的原评论作第二来源。容器里:`docker compose exec multireviewer node src/recover-finding-content.ts --repo <owner>/<仓库>`,确认预览后再加 `--apply`
...
- `MULTIREVIEWER_DRAIN_TIMEOUT_SECONDS` — 优雅退出时等在跑的轮次的上限秒数(issue #249),默认 300。收到 SIGTERM / SIGINT 后服务停止接受新投递与重跑,在跑的轮次跑完当前批次并落库后退出;到这个上限仍没停下的轮次不再等,日志记下放弃了谁,它们停在哪一批已经落库,下一次启动续跑(issue #248)。**它与 `docker-compose.yml` 的 `stop_grace_period` 必须是同一个数**,compose 那边短了的话进程会在等批次的途中被 SIGKILL,那一批白跑
...
- 2026-09-06: 落地 issue #268。**恢复已有评审记录中可追溯的影响与修改建议**:新增一次性操作入口 `src/recover-finding-content.ts`(见「常用命令」),默认只读预览,`--apply` 先留库快照再一个事务写入。来源只认唯一对应:同一轮同一模型对同一文件同一段问题表述、成功上报的轨迹内容恰好一种才补,两种即跳过;轨迹没有时只认这一行自己发出去的原评论(折叠到旧评论上的行不算);按复核结论合成的延续那一行沿已落库的延续关系从上一处抄历史说法。只补 NULL,非空不碰;不调用模型、不重跑、不写 Forge。细节见 `src/AGENTS.md`。
- 2026-09-06: 落地 issue #267。**历史 Finding 延续时保留影响、修改建议及出处**:复核判仍在、代码位置已变而本轮没有重报时,合成的延续 Finding 把历史各模型归属的问题、影响与建议原样带到新位置,每段记原模型与来源轮次的 head;新 Gitea 评论与面板都标注「沿用 X 在 <head> 上的说法,尚未针对新代码重新验证」,不署给给出新位置的模型,也不算进它的归属与统计;本轮重报的用本轮内容。历史说法另落一张表,多轮延续出处仍是最初那一轮。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
...
- 2026-09-05: 落地 issue #261(ADR 0027)。**Pi 内置模型服务按模型绑定调用目标,兼容 OpenRouter 多协议**:此前整家内置服务只取 Pi 内置表首行模型的协议与地址,Pi 0.85.0 换了 OpenRouter 的首行就会让存量服务整体判「目标已变化」、而且所有模型只能走一种协议。现在每个模型用自动目录里它自己那一行保存的协议与地址,同一家服务可以同时含 Chat Completions 与 Anthropic Messages;模型服务版本把去重排序后的调用目标集合记成目标绑定,候选预览列出本次要绑定的目标;目标只经显式验证与新版本提交进入绑定,目录刷新不改绑,刷新拉进来的新目标在这一版里标「调用目标未经验证」;升级前的版本只延续能凭指纹证明的那一个目标,证明不了即待重新验证并在模型调用前拒绝使用凭据,禁止凭 Pi 首项变化自动改绑;手填 model id 优先用该模型可确认的目标,只有一个已确认目标时沿用,混合协议下明确拒绝并指向自定义模型服务。Review Run、规则 agent、合并 agent、基点探索与面板共用同一份解析,运行目标随版本冻结。`model_service` 多一列目标集合。存量 OpenRouter 服务升级 Pi 后照常运行,刷新并重新验证一次即绑上两种协议。决策见 `docs/adr/0027-builtin-model-service-targets-are-bound-per-model.md`,术语见 `CONTEXT.md` 的「调用目标」「目标绑定」,细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。

- 2026-09-05: 落地 issue #259。**审查轨迹里每个 Reviewer 的「收尾」块标题改写结束事件的摘要**:完成时写「报出 N 条 · 契约拒绝 N 次 · 锚定被拒 N 次 · N tokens」(为 0 的被拒项省略),失败时写失败原因第一行加「失败」徽章,结束事件未到时只有「进行中」徽章;分批块与旧轨迹的「N 段文本 · N 次工具」不变。只改面板,细节见 `web/AGENTS.md`。

- 2026-09-05: 落地 issue #254。**模型服务发现的远程目录缓存移出版本库**:`.cache/worktrees/pi-models/models-store.json` 是显式发现按需重建的派生输入,此前被版本库跟踪了一份 8,567 行的副本。取消跟踪并在 `.gitignore` 加精确规则(只覆盖这一份文件),已有工作区里的缓存文件保留,`.dockerignore` 此前已排除 `.cache`。新检出的仓库仍由 Pi 内置目录完成发现;数据库里的模型服务版本与目录快照照旧是产品事实,Review Run 继续使用已固定的模型服务版本。
...
- 2026-09-05: 落地 issue #253(ADR 0024 修订)。**完整批次计划在开跑时冻结落库,续跑核对全部批次的分组**:`review_run` 多一列批次计划(全部批次的文件清单,开跑时与历史快照同一次写下,之后不改;升级前的轮次读回为没有计划)。服务重启后的续跑改为对照这份计划核对重新切批的每一批,已完成与未完成的批次都要相符;总批数与已完成首批相同、只有未完成部分分组变了的那种情形(停机期间改了分批上限)从此认得出来,改判失败并把「分组与冻结计划不符」写到轮次级失败原因列,不启动 Reviewer,也不把改变后的分组混进原轮次。没有计划的升级前中断轮次按同一条规则改判并写原因,已完成的历史轮次不变。原计划成立时只跑缺结果的批次,已完成批次不再调 Reviewer,轮次编号、历史快照、模型顺序、结果序号、用量汇总与单次发布行为不变;排空行为不变。`CONTEXT.md` 的「续跑」词条与 ADR 0024 的核对项跟随更新。细节见 `src/AGENTS.md`。

- 2026-09-05: 落地 issue #252(ADR 0025)。**延续的交接改在新评论发布确认后完成**:Review Run 收尾的顺序改为发布 review 并读回评论标识 → resolve 旧评论 → 落库延续,新评论正文里「延续自」那句照旧。发布明确失败的那一轮不 resolve、不记延续,旧 Finding 留在未处置,本轮 Reviewer 结果与 Finding 照常落库,失败原因写在轮次上(#256 的那一列)并进轨迹,不再表现为成功完成;发布成功而旧评论关不上的延续照记,旧行带「交接未完成」标记,旧评论留在 Forge 上可处置,下一轮收尾时自动重试关闭,回填读到它已关闭也算交接完成;发布结果不确定(Gitea 创建成功但读回评论失败,或创建请求没有响应)按发布失败处理,原因带已取得的 review id 供人核对,任何路径都不自动重发。`finding` 多一列交接标记,面板在 Finding 行与轨迹上呈现「交接未完成」。PR 与范围审查、完整审查与只复核、三档延续判据共用这一套顺序。`CONTEXT.md` 的「已延续」词条补交接未完成。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。

- 2026-09-05: 落地 issue #256(ADR 0026)。**轮次级失败原因落在 Review Run 自己上,与 Reviewer 失败分开记**:`review_run` 多一列失败原因(NULL 即收尾正常,升级前的轮次读回为无失败记录),`failed` 列的含义与取值不动;服务重启后改判的中断轮次同时写这一列与逐 Reviewer 的失败行,零 pin 的轮次从此也读得到原因;轨迹多一档 Run 级事件 `run_failed`,带同一句原因。Store 暴露只写这一列的入口,给 #252(发布 review 失败)与 #253(续跑计划不符)的收尾路径用,本票不改它们的行为。面板的轮次列表分「完成 / 运行失败(全部 Reviewer 失败) / 收尾失败」三态并摊出原因,轮次详情与时间线同样呈现。启动续跑的选择条件不读这一列。`CONTEXT.md` 新增「轮次失败」词条。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
...
- 2026-09-04: 落地 issue #247。**服务重启后中断的 Review Run 不再永远显示「进行中」**:启动时把所有没有结束时间的轮次改判失败,结束时间取启动时刻,失败原因是「服务重启,上一轮没跑完」,并按这一轮已固定的 Reviewer 逐个落在轮次详情的模型行上——与其他失败一轮同一种看法。被改判的轮次在 PR 上残留的 👀 随即撤掉,撤不动只记一行日志,不阻塞启动也不影响改判。面板从此不再对这些轮次轮询,轨迹页照实停在崩溃前最后一条事件,逐轮「重新运行」入口对它可用。(**这一段已被 issue #248 改写**:启动时先尝试续跑,续跑不成立才走这里的改判;逐轮「重新运行」仍是从头开新一轮。)没有中断轮次时启动路径零改动,一次 Forge 都不问。工作副本准备(issue #184)与基点探索(issue #205)早有同样的规则,这一票把 Review Run 补齐。细节见 `src/AGENTS.md`。

- 2026-09-04: 落地 issue #240–#245 之后的两轴评审复核修复:合并 agent 折叠进旧条的那一行沿用旧条指纹,阶段汇总里同一处问题只占一行;只复核重跑的 409 闸与本轮变更文件求交,历史全在没改的文件上时不再先答已触发;只复核那一轮的改动行数与文件数同一口径;轨迹面板认折叠与延续两档事件。详见 `src/AGENTS.md` 与 `web/AGENTS.md`。

- 2026-09-04: 落地 issue #244(spec #238)。**锚在删除点旁边的 Finding 显示删掉那几行的提交与作者**:离落点最近的那处改动是删除点(旧侧被删的行在新侧对应的位置)时,行作者取 base..head 里删掉那几行的提交,同样带「相邻改动」标记;新增行与删除点在同一个距离比较里,等距取上方。删除提交找不到时那一条留空、日志一行,其余照常。库结构、接口契约与前端都没动。细节见 `src/AGENTS.md`。

- 2026-09-04: 落地 issue #241(spec #238)。**落在上下文行上的 Finding 不再把几个月前的提交当责任人**:落点是本轮 diff 里的改动行时行作者一字未变,是 hunk 内的上下文行时改取同一个 hunk 内离它最近的那处改动(同距取上方),这一档带「相邻改动」标记随行作者落库,阶段汇总投影带出去,Finding 卡片在行作者之后写明。判不出来仍留空、日志一行;存量不回填,读取时的补录口径不变。`CONTEXT.md` 的「行作者」词条补上这一档。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-04: 落地 issue #245。**重跑默认只复核历史,勾上「完整审查」才新报**:`POST /rerun` 两种入参各多一格非必填的 `mode`(不给即只复核,`full` 是完整审查,别的取值 400);没有未处置历史的阶段只复核回 409,一轮不开。模式随这一轮落库,时间线上只复核的那一轮带标记,轮次侧滑与本轮指令并列显示它。不新增权限格,随 `review:rerun` 走。CONTEXT.md 补「只复核」词条;详见 `src/AGENTS.md` 与 `web/AGENTS.md`。
...
- 2026-09-04: 落地 issue #243(父 spec #237)。**代码改写之后,同一个问题被延续到新位置而不是新开一条**:合并 agent 判定本轮某条与一条历史是同一回事、而那条历史所指的代码已经改写时,本轮那条承接同一条 Finding Identity——旧评论关掉并记「已延续」,新评论正文带「延续自」的链接,处置记录因此连续。语义判断压过原有的两道机械判定:同一条历史既被 agent 命中又被词法配对或复核结论的新位置盯上时,以 agent 命中的那条承接;一组命中两条历史时只有先来的那条(id 小的)交接,另一条保持原状。合并 agent 跑不成时延续退回词法配对,与此前一致。ADR 0022 补一段带日期的修订附记,把「合并只看本轮」这条前提的变化记在案。细节见 `src/AGENTS.md`。

- 2026-09-04: 落地 issue #240(父 spec #237)。**同一处问题换个说法再报时,被折进旧评论而不是新开一条**:合并 agent 归组时一并拿到同文件的历史 Finding(未处置与已处置都给,只取本轮有 Finding 报出的那些文件),判定本轮某条与一条历史是同一回事、而那条历史所指的代码原样还在时,本轮那条折叠到旧评论、不再新发;命中已经处置过的旧条即沉默。此前跨轮次去重只有指纹一道,增量在指纹窗口里插几行就折不上,同一个问题于是两轮各挂一条。含历史成员的组不再验行距——代码改写之后问题会漂得很远;不含历史的组照旧三条硬性质全验,方案不成立、agent 失败或超时一律整体回退到指纹折叠,最坏情况与此前一致。轨迹上每次折叠记明判据是指纹还是合并 agent,后者带它给的那句理由。细节见 `src/AGENTS.md`。
...
- 2026-09-04: 落地 issue #249。**优雅退出**:进程收到 SIGTERM / SIGINT 后进入排空,HTTP 停止接受新连接,新投递与面板重跑回 503 且不开跑、不占幂等键,在跑的轮次跑完当前批次并落库后停下,全部停下即退出,退出码 0。退出点取批次边界(ADR 0024):停在边界上的轮次没有结束时间,由下一次启动续跑(issue #248)。排空上限是新的可选环境变量 `MULTIREVIEWER_DRAIN_TIMEOUT_SECONDS`(默认 300 秒),与 `docker-compose.yml` 新增的 `stop_grace_period: 300s` 是同一个数,改一处必须改另一处;到上限仍没停下的轮次不再等,日志记下放弃了谁。没有进行中轮次时 SIGTERM 立即退出。发版因此最长等一个批次,细节见「部署」。
