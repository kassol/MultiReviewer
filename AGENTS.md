...
- `MULTIREVIEWER_CACHE_DIR` — 工作副本缓存根目录,默认 `.cache/worktrees`。镜像里是 `/data/worktrees`。模型目录的远程增量也缓存在它下面的 `pi-models/models-store.json`,而且 Reviewer 子进程也读这一份(只读,不联网):面板选得出的远程模型靠它才在子进程里取得到。这个目录因此对服务进程必须可读可写,换掉或清空它只影响下一次读目录,不丢数据
