...
- 2026-09-09: 落地 issue #309(父 spec #305,ADR 0030)。**阶段详情按同根因组折叠展示,组卡上一次处置整组**。阶段汇总接口多两格:每条 Finding 上可空的同根因组引用(组 id、根因说明、成员数、组内次序)与这个阶段的组列表。组属于轮次、每轮重新提,因此只取**最新一轮**的组;成员记的是落库当时那一行,读的时候沿 Identity 与延续链映到此刻列表里的那一条,映不过去的不进组。合并 agent 缺席的那一轮回空列表,面板不渲染组。新端点「处置同根因组」(`POST /stages/{stageId}/root-cause-groups/{groupId}/dispose`)权限格沿用 `finding:dispose-batch`,不为它多一格:组内当前未处置的成员逐条走与逐条处置**完全相同**的落库、Forge 回写与处置反哺(同一个函数),已处置的跳过、不覆盖人已经做过的决定,响应列出写了哪几条、跳过哪几条、哪几条失败;单条处置不联动同组。面板上入组的条目折进组卡(标题即根因说明,默认折起,展开后成员与组外条目长得一样),组卡上有「处置整组」,Forge 评论尾行的 `?rootCause=` 链进来时那一组展开并滚到视野正中。存量不回溯,无迁移。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-09: 落地 issue #308(父 spec #305,ADR 0030)。**同根因组:一轮里出自同一个根因的几条 Finding 归成一组,Forge 评论末尾多一行指向其余各处**。合并 agent 在同一次调用里、归组全部完成之后调第二个工具提出同根因组:成员以本轮合并组的次序引用,附一句中文根因说明。验收独立于分组方案——成员至少两个合并组、编号真实存在、一个合并组至多进一组,任一条不满足即只丢这一组并在审查轨迹记一条 `root_cause_group_rejected`,分组方案照常生效。组与成员随收尾那一笔事务落进两张新表(`root_cause_group` / `root_cause_group_member`,随 `STORE_SCHEMA` 建,旧库打开即建,无迁移);成员是最终落库的那一行——本轮新报的、折叠到的那条历史行、延续承接后的新行。入组的 Finding 的 Forge 评论末尾多一行「同根因另见 N 处」并链到面板该阶段详情、定位到组(`?rootCause=<组 id>`),未入组的正文逐字不变,折叠的那些本轮不发评论因此也不多这一行。组内每条 Finding 的正文、严重度、分类与处置都不因入组而变;合并 agent 缺席、失败或方案没过验收的那一轮没有组。面板按组折叠展示与组级处置是 issue #309。服务端细节见 `src/AGENTS.md`。
- 2026-09-09: 落地 issue #307(父 spec #305,ADR 0030)。**Finding Identity 成为「同一处的同一问题」**:跨轮次折叠改以合并 agent 的判定为准,指纹只在它缺席、失败、超时或方案不过验收时兜底。交给合并 agent 的历史清单每条多一行位置提示(它的指纹在本轮 head 上命中了本轮哪几条 Finding),系统提示词写明同一处不等于同一问题、提示是证据不是结论。agent 把本轮某组与一条历史归为同一组即折叠或延续(与此前逐字一致);它没把指纹命中的历史归进任何组,本轮那条就是同一处的新 Finding——用本轮落点的指纹落库,不继承旧条的处置、评论与状态,照常发行级评论、进阶段统计。**已处置(含已驳回)位置上的新问题从此会发出评论**;某处代码本身「不用报」该走评审规则压掉,不再指望位置上的旧驳回。阶段统计里 Identity 的键从「文件 + 指纹」改成承载它的那条 Forge 评论,同一「文件 + 指纹」下因此可以有两条 Identity,处置率与参与条数的口径不变。审查轨迹多一档「同一处的新问题」(带历史 id 与 agent 的理由),面板轨迹页渲染它。本轮内的合并、存量数据与升级步骤都不变:不加列、不加迁移,旧库直接打开。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-09: 落地 issue #306(父 spec #305,ADR 0030 的第三件事)。**范围审查分批时 Reviewer 只报本批文件**。分批 Reviewer 的任务提示词多一句「只对列出的这些文件报出,其他文件仅作阅读上下文」;编排层收每批结论时按该批文件清单过滤,批外文件的报出在锚定与合并之前丢掉——不落库、不发评论、也不进合并 agent,只在审查轨迹里留一条 `finding_out_of_batch`(文件、行、标题、模型、批次)。它与「锚不进本轮 diff」的 `finding_discarded` 是两档不同事件:拦下它们的是两条不同的规则。批外文件自有带着它们 diff 的那一批去审,那一批的落点才有依据。PR 触发的单批审查逐字不变。服务端见 `src/AGENTS.md`,面板见 `web/AGENTS.md`。
- 2026-09-09: 决策记录,**三件事已全部落地**(只报本批文件见 issue #306、Identity 见 issue #307、同根因组见 issue #308,面板按组展示与组级处置见 issue #309,均在上面几条)(去重缺口收尾,spec #305)。**Finding Identity 改为「同一处的同一问题」,跨轮次折叠由合并 agent 判定、指纹只兜底;合并 agent 顺带提出同根因组;范围审查分批时批外文件的报出丢弃并记轨迹**(`docs/adr/0030-finding-identity-is-the-same-problem-at-the-same-spot.md`):指纹命中的历史交给合并 agent,归为同一组才折叠,没归即同一处的新 Finding、正常发评论、不继承旧条处置;合并 agent 不可用时退回指纹折叠。同根因组是合并组之上的另一层:成员来自本轮至少两个合并组、不限文件,附一句根因说明,组内各条 Finding 的 Identity、正文、评论、处置都不变,只多面板按组展示、组级一次处置未处置成员的入口与 Forge 评论末尾一行「同根因另见 N 处」;组每轮重新提、不跨轮保持。存量不回溯。CONTEXT.md「Finding Identity」词条改口,新增「同根因组」词条,ADR 0015 / 0022 顶部附记指向 0030。
- 2026-09-09: store 与 review 收缩(ponytail 审计 #1/#3/#11/#27/#32/#42/#44)。**一次性操作入口与已跑完的迁移全部删除**:`src/recover-finding-content.ts` 与 `src/review/recover.ts`(issue #268 的影响与建议回填,线上已跑过 `--apply`)连同它们的用例与本文的命令、目录索引两条一起删;`openStore` 里五处整表重建(`finding_attribution` 去唯一约束、`rule_proposal` 出处下沉与合并变更类型两次、`rule_trace` 与 `rule_proposal_source` 的来源 CHECK 各一次)与 `rule:write` → `knowledge:write` 改名删掉,`ADD_COLUMNS` / `ADD_INDEXES` 与吞「列已存在」的 try/catch 也删掉——尚未声明的 13 列与那一个索引按原来的类型、默认值与 NOT NULL 并进 `STORE_SCHEMA` / `MODEL_SERVICE_SCHEMA`,新库建出来的表逐列逐索引与删之前一致。**保留**「逐项版本键合并成 `settings_version`」那一条(issue #301,2026-09-09 才发版)与 `PRAGMA user_version` 那道骨架版本判断。**后果:发版之前落的库备份不能再直接用新镜像打开**——缺的列不会再被补出来,要开旧备份就得连镜像一起回退到带补列的那一版;唯一的部署库跟着这次发版走,不受影响。`Store` 去掉只有用例调的 `addReviewRule` 与 `putGlobalSettings`(连同 `MANUAL_RULE_ORIGIN`),播种改走新的 `test/support/store-seed.ts`(整页替换与直写 `review_rule` 行);`trace.ts` 的手写频道换成 `EventEmitter`,`drain.ts` 的等待者集合换成单个 resolver,`carriedByFinding` 换 `Map.groupBy`,两处取末位换 `.at(-1)`——订阅、排空与读回的语义一律不变。造旧库的用例(`persistence` 十条、`panel-permissions` / `panel-repos` / `panel-revision-intents` 各一条、`panel-rule-proposals` 两条)随迁移一起删。
- 2026-09-09: reviewer 与部署脚本收缩(ponytail 审计 #12/#16/#26/#38/#39/#40/#43/#46/#47)。三个子进程从建会话到退出那一段收成 `reviewer/worker-tools.ts` 的 `runAgentWorker`,`numbered-read.ts` 并进同一个文件;目录只留 `models`,`remote` 与 `vendors` 两个字段作废(ADR 0009 顶部附记);`AnchorResult` 去掉没人读的 `corrected`。`scripts/setup.sh` 删掉废弃变量的自动清理与备份,以及从未调用的 `confirm()`。IPC 消息序列、退出行为与向导问出的那几项一格未动。
- 2026-09-09: 面板服务端收缩(ponytail 审计 #5/#13/#21/#23/#30/#35/#36)。`webhook/server.ts` 的 74 条面板路由压成每条一行,method / pattern / access / assignment / handler 逐条不变(比对脚本按运行时对象核过);五条模型服务写端点的共同前奏(主密钥、请求体、这一版还在、版本对得上)收成 `loadVersionedService`,八处「写没成 → 重读版本 → 409」收成 `staleVersionConflict`,措辞与状态码逐字不变;查询串上的整数参数收成 `intQuery`;`Object.freeze` 九处与 `frozenRuntimeModel` 删掉(运行计划的类型本就 `Readonly<>`,没有依赖冻结的运行时判定),对 `supplements` 的恒等 `.map` 投影改成直接传原数组或按需浅拷贝。`panel/auth.ts` 的 `PanelAuth` / `PanelAuthOptions` / `LoginOutcome` 三个手写接口删掉,`createPanelAuth` 直接返回那一次登录判定的函数,`PanelAuth` 改为跟着它推导的别名。行为等价,无接口变更。
- 2026-09-09: 面板前端收缩(ponytail 审计 #6/#20/#28/#37/#41/#45/#48)。十二处 `AlertDialog` 确认块收成 `web/src/components/confirm-dialog.tsx` 一个 `<ConfirmDialog>`;`api.ts` 新增 `send<T>(path, method, body?)` 封装最常见的「发请求 → 报错 → 解析 JSON」形状,替掉 26 处重复与 access-control/credentials 里两份近似的 `responseJson` 本地包装及三个命名请求函数;`styles.css` 删除 14 个从未作为 class 或 `var()` 引用过的旧色别名(`--card`、`--secondary`、`--accent` 等,`--foreground` 因 `index.html` 仍在用而保留);`credentials.tsx` 的 `navigateBack` 四分支合成一次 `to` 再统一 `navigate` + `restoreScroll`;`repo-actions.tsx` 的注册仓库搜索框改用 `commit-picker.tsx` 已导出的 `useDebounced`;`index.html` 删除随每次响应下发、内容与当前构建脱节的设计简报注释。审计里的滚动与焦点归还两条(#15、#19)核实后判定不成立予以保留,理由见 `web/AGENTS.md` 对应条目与 `src/components/use-dialog-return-focus.ts`。行为与外观逐字未变,`pnpm --filter @multireviewer/web typecheck/build/test` 分别过。
- 2026-09-09: 测试夹具去重(ponytail 审计 #4/#7/#9/#10/#14/#17/#18/#24/#25/#29)。收拢 `test/support/`:面板 harness 的 `cleanups` 数组改由 `testCleanups()` 预注册、`startPanelHarness`/`startReadyPanelHarness` 去掉该参数——`testCleanups()` 交的是文件级的同一份队列,`after` 钩子在模块加载时挂好,整份队列在测试文件跑完时一次清完;钩子挂进单个用例会把清理提前到那个用例结束,后台的工作副本准备还在写缓存目录与临时库,清理循环因此 `ENOTEMPTY` 中断、面板服务的 `close()` 轮不上,整份测试文件跑完不退出;`panel-harness.ts` 新增 `scopedUser`/`seedRepo`/`userCookie`/`startRangeReview`(`startReadyPanelHarness` 配套加 `registerRepo` 选项)与内部 `counter()` 工厂(收拢六路后台回调的等待三件套),`factoryCalls`/`snapshots` 两个纯投影字段删除、断言处现算;`batch-run.ts` 的 `setup()` 加可选参数、`query()` 原样导出,替掉四份本地复制,`memory-forge.ts` 新增 `scriptedRuleAgent`。各测试文件里同形的本地实现改为委托或直接导入,行为不变,断言与用例名未改。审计 #18 的 `seedActiveEntries`(调用 `Store.addReviewRule`,由 #27 一并删除)本轮不动。
- 2026-09-09: **spec #300 四票(#301–#304)与两轮两轴评审修复全部落地、发布 00-test 并在 zhangxu/review 端到端验收通过**(发布前库备份 `data/multireviewer.db.bak-20260909-300`)。迁移核对:整体版本键 `settings_version` 就位、六个逐项版本键已删、`repo.auxiliary_model` / `repo.settings_version` / `review_run.auxiliary_model` 三列到位,存量仓库配置原值不变。验收里真实发起一次知识整理与一轮 PR 触发的 Review Run:整理轨迹首事件与轮次落库的辅助模型都是仓库覆盖那一处。验收后三处面板文案与样式修正见 `web/AGENTS.md`。
- 2026-09-09: spec #300 第二批(#303-#304)两轴评审修复。**行为修正三处**:①合并 agent 与四条知识链路问同一份「跑得动」判据(`auxiliaryModelRunnability`)——此前合并那一侧只看凭据与运行模型,辅助模型选了这个模型不支持的档位时照样开跑,跑的是运行侧 clamp 出来的邻档;现在同样缺席,这一轮的合并走算法档;②事务内的模型引用兜底改调 `listModelReferences` 那一份清单,只被全局或某个仓库的辅助模型引用的模型,删自定义服务与摘唯一来源补录一样拦得住;③仓库配置弹窗里辅助模型的「自定义」不再因为全局与模型组合都空而禁用,仓库管理员可以从空选择起步给自己这个仓库先设一处。**前端去重**:思考档位控件收成 `web/src/components/thinking-level-picker.tsx` 一份(模型组合编辑器与辅助模型控件共用,行为与文案不变),`AuxiliaryModelLine` 从 `repo-rules.tsx` 挪进 `web/src/components/` 并在仓库配置弹窗里替掉自写的来源与档位文案,「引用带上一档」收成 `lib/model-ref.ts` 的 `modelRefWithLevel`;`lib/model-ref.ts` 里两处裸 NUL 字符换成 `\0` 转义。**测试与命令**:`pnpm check` 末尾接上 `pnpm --filter @multireviewer/web test`,前端纯函数因此进常规验收;服务端补三条用例(辅助模型独占引用时两条删除路径被拦、档位不支持时不建合并 agent、仓库辅助模型档位被拒),前端纯函数补一条。三份 `AGENTS.md` 的测试边界与「跑得动」判据说法跟着改口。
- 2026-09-09: 落地 issue #304(父 spec #300,ADR 0029),**spec #300 的四票至此收口**。**合并 agent、处置反哺与人工提议接上辅助模型,选模型的隐式规则全部删掉**。此前两条规则藏在代码里:合并 agent 取配置序第一个 Reviewer,处置反哺与人工提议取这个仓库最近一次基点探索用的模型、没探索过再取全局组合第一个——发起一次探索换了模型,之后每一条处置备注的解读都悄悄跟着换,写备注的人不知道。现在五处 agent 工作都调同一处解析(仓库覆盖 ?? 全局 ?? 生效组合第一个)。合并 agent 在 Review Run 开跑时解析一次,**随轮次落库并冻结**(`review_run` 多一列辅助模型):开跑后改配置追不上这一轮,续跑沿用落库的那一处而不重新解析,它也不进续跑校验清单——改了它只影响下一轮,不该让跑了一半的轮次续不成;解析结果跑不了(缺凭据或缺运行模型)即缺席,这一轮的合并走算法档,与升级前一致。两处配置都没设时解析退回生效组合第一个,与升级前合并 agent 的取法逐字相同,**升级零操作**。处置反哺与人工提议不再看探索记录,探索与整理记录里的模型列只作历史;选不出或跑不了时那句失败原因改口,指向审查策略与仓库配置,提交修订意图那一句 409 同律。修订意图框上的只读模型展示改读 `GET /repos/{id}/auxiliary-model`,与发起探索、发起整理那一行是同一个组件、同一个结论;`GET /repos/{id}/rules` 不再回 `intentModel`。详见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-09: spec #300 第一批(#301–#303)两轴评审修复。**行为修正六处**:①「模型组合首次配置后非空」在 `PUT /settings` 与 `store.replaceGlobalSettings` 收成一条判据(`storedReviewersEmpty`)——库里现存的组合是空的或从没配过时照收空组合(只落上限与等级),配过非空之后不再收空,审查策略页也不再因此禁用保存;②`PUT /settings` 的整份对象每一项都要给,缺任一项 400,不再把缺项当「跟随默认」悄悄清掉;③`PUT /repos/{id}/settings` 版本先核、再判模型可用性,表单过期时一律回带 `current` 的 409;④仓库那一块的模型可用性与辅助模型可用性都只在换了的时候判(端点与 `store.putRepoSettings` 两处同律),覆盖里早就有失效模型时只改等级或辅助模型照常保存;⑤升级迁移删掉逐项版本键的同时把整页版本键建成 1(幂等);⑥辅助模型计入模型引用(`listModelReferences` 多 `global-auxiliary` / `repository-auxiliary` 两种位置),删凭据、删服务、切目标与摘唯一来源补录都按它阻止,模型服务概览的引用清单也列出它;冲突自定义 provider 改名时,全局与各仓库的辅助模型引用跟着模型组合一起在同一事务里换 provider。**清理**:`store.setRepoReviewers` / `setRepoMinReportSeverity` 两个无端点方法删除,夹具改走 `putRepoSettings`;仓库 409 分支里对 `getRepo` 结果的三层 `?? 0` 兜底去掉。**面板**:上限项补回 `aria-describedby` 与逐项错误文案、删掉与 `HelpTooltip` 重复的段末说明、离开确认补 `onOpenChange` 让 Esc 关得掉;两页各写一份的草稿比较收成 `web/src/lib/model-ref.ts` 一份纯函数(`sameModelRef` / `sameModelRefs`,辅助模型走同一条),`web` 包补 `test` 脚本跑 `src/**/*.test.ts`。ADR 0011 顶部附记作废「模型组合与分批上限仍独立保存」,CONTEXT.md 审查策略词条改口「各项各自取值、整页一次保存」。
- 2026-09-09: 落地 issue #303(父 spec #300,ADR 0029)。**辅助模型进审查策略与仓库配置两页,基点探索与知识整理不再选模型**。审查策略「模型」段多一处辅助模型(一枚模型单选加一枚思考档位),空着即「跟随模型组合第一个:<模型标识>」;仓库配置弹窗多第三项「辅助模型」,与模型覆盖、最低报告等级同形的「跟随全局 / 自定义」两态,自定义从当前生效值起步,随整块一起保存。两处控件复用模型组合同一份候选投影(`GET /model-services`)与同一套档位规则,只收当前可用的模型与它支持的那几档,不可用或档位不支持即整份拒收并指名是辅助模型那一项。发起基点探索的表单只剩基点选择,其上一行只读写着「将使用：<模型标识> · <档位>（来源：仓库覆盖 / 全局 / 模型组合第一个）」;发起知识整理变成带同一行文字的确认;生效模型跑不了时两处的发起按钮禁用,那句话说得出原因与去哪里改。接口上审查策略的整份对象多一项 `auxiliaryModel`(可空,随 #301 的整页读写与版本走),`PUT /repos/{id}/settings` 的请求体多第三项 `auxiliaryModel`(三项必给,可空即跟随全局),新增只读投影 `GET /repos/{id}/auxiliary-model`(登录加仓库分配即可读,回引用、来源、可用性与不可用原因),`GET /rule-models` 删除、路径不匹配即回「没有这个端点」;两条发起端点的请求体不再收 provider / model / thinkingLevel,带了即 400,生效模型选不出或跑不了即 409。**生效解析只有一处函数** `store.resolveAuxiliaryModel(repoId)`:仓库覆盖 ?? 全局 ?? 这个仓库生效模型组合(仓库模型覆盖 ?? 全局组合)的第一个,返回值带来源;合并 agent、处置反哺与人工提议改调它是 issue #304。探索记录里的模型与档位两列继续写实际所用的那一处,只作历史。升级零操作:仓库多出来的辅助模型覆盖列与审查策略里那一项都从空起,解析退回组合第一个,与升级前的取法一致。详见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-09: 落地 issue #302(父 spec #300,ADR 0029)。**仓库配置弹窗整块一个保存**。原来模型覆盖与最低报告等级各是一个端点、各自即改即写:点一下「跟随全局」当场清库,自定义态下的下拉每选一档存一次,没有取消,也没有版本号——两个人同时改同一个仓库就互相覆盖,而且谁都不知道。现在两项进同一张表单,各自「跟随全局 / 自定义」两态,切到自定义从当前生效值起步,改动只留在表单里;底部固定「保存 / 取消」,保存一次写两项并把这个仓库的整块版本号加一。有未保存改动时关弹窗(取消、遮罩、Esc)先弹一次确认;别人先改过时保存被 409 拦下,表单换上服务端当前值作新基线、人的改动原样留着,提示核对后再保存。准入 Key 轮转与工作副本两块是动作,按钮与端点一字未动。接口上 `PUT /repos/{id}/reviewers` 与 `PUT /repos/{id}/min-report-severity` 删除,路径不匹配即回「没有这个端点」,新端点是 `PUT /repos/{id}/settings`(收 `reviewers`、`minReportSeverity` 与 `expectedVersion`,两项可空即跟随全局,权限 `repo:write`、按仓库分配收窄);辅助模型覆盖是这个请求体的第三项,随 issue #303 落地。升级零操作:仓库多出来的整块版本号从 0 起,两项覆盖的存量值一字不动。详见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-09: 落地 issue #301(父 spec #300,ADR 0029、ADR 0010 附记)。**审查策略整页一个保存**:模型组合、四项运行上限与最低报告等级进同一张表单,一颗「保存」、一次写入、一个版本号。改三项点一次;没有改动时「保存」与「放弃改动」都禁用,有改动时动作条写「有未保存改动」。上限项与报告等级留空即跟随系统默认——「取值来源」标记删掉了,值空不空就是来源,占位符与下拉里的「系统默认」写着那个默认值,每项旁的「恢复默认」只清空字段、随整页一起提交。别人先改过这一页时保存被 409 拦下,表单载入服务端当前值作新基线,用户的改动原样留着并提示核对。有未保存改动时站内切页弹确认,关标签页走浏览器原生提示;只读态把三段以静态值铺开。服务端 `GET /settings` 回整份对象加一个版本号与一份 `defaults`,`PUT /settings` 收整份对象加 `expectedVersion` 全量替换,任一项校验不过整份一项都不写,版本不符 409 并带回当前整份对象。失效模型仍只门禁模型组合本身的写入:组合原样未动的那一次照常保存得下,上限与报告等级不被连坐。存储仍是 `global_setting` 键值表,值一格不动;升级前的逐项版本键在开库时一次性删掉,整页版本从 1 起算,幂等。审查配置就绪与首次配置检查单的判据不变。**辅助模型这一票不做**(#303),表单的「模型」段留着它那一行的位置。详见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-09: 决策记录,尚未落地(spec #300)。**选模型的地方收成审查策略与仓库配置两处,Reviewer 之外的 agent 共用一处辅助模型**(`docs/adr/0029-one-auxiliary-model-slot-for-all-non-reviewer-agents.md`):合并 agent、基点探索、知识整理、处置反哺与人工提议都用它,仓库配置可覆盖(repo:write),两处都没设即退回生效组合第一个;探索与整理的发起表单不再选模型,反哺不再看探索记录。同一批设计决定:审查策略页与仓库配置弹窗都改成整页一个保存、一个版本号,运行上限四项与最低报告等级改为可空即跟随系统默认(不再存来源标记),策略页有未保存改动切走时拦下提示,仓库那两个覆盖端点合成一个 `PUT /repos/{id}/settings`;模型服务页的验证模型不在范围内。CONTEXT.md 新词条「辅助模型」,审查策略、思考档位、综合说明、基点探索、知识整理、人工提议六个词条改口。
- 2026-09-09: 落地 issue #299(父 spec #293,ADR 0028),**spec #293 六票至此收口**。**人从此不手写陈述、型与作用范围**。原来知识集有四个直改面——新增条目、修改条目、往草案里手填一条、逐条改草案——加上采纳一条提案时能带改后内容,五处都让人填同一张三字段表:型、陈述、一个没有校验的 glob。四条端点整个删掉,路径不匹配即回「没有这个端点」;采纳端点连正文都不读,带了也忽略,落进知识集的就是队列里被裁决的那一份。面板上那张表(`RuleForm`)与「新增知识条目」「修改」「向草案新增」「改后采纳」四个入口一并撤掉,写内容只剩弹窗顶部的意图框与条目、提案、草案条目卡片上的「改写」。**不写内容的动作一个没动**:直接废止一条条目、删除一条草案条目、知识确认、驳回与批量裁决照旧一键。人手填事实那道 500 字上限与只为那张表存在的解析随之删除,陈述长度从此只剩 agent 产出那一道 100 字的闸——两套上限并存的局面结束。存量 `manual` 出处的条目原样保留,升级零操作。详见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-09: spec #293 那一批(issue #294–#299)的两轴评审修复。**四处行为修正**:一、目标为一条待裁决提案的意图,在 agent 一条指向目标的产出都没给时也先查那条提案还在不在待裁决队列里——不在即意图失败并留原因(与目标为条目、目标为草案条目两档同一口径),待裁决才算零产出完成;此前这一档会把「目标已经被裁决掉」报成完成,人以为 agent 认为不用改。二、目标为一条知识条目的意图,并入候选只取指向它的**非废止型**待裁决提案:废止型说的就是废止哪一条,并进去等于把它改成别的意思,与提交时那道 400 同一口径;agent 仍指名它的话那一条产出被丢掉并在知识轨迹里说明原因。三、知识集未确认时产出追加进草案那条路径只对无目标的意图成立:草案条目的出处恒是人工提议,带备注的处置在未确认仓库上的产出因此改为照常排进修订提案队列并带处置反哺附注,草案不动。四、面板意图行完成后的产出从「产出 N 条」换成逐条可点的「提案 #id」与「草案 #id」,点它切到那一档所在的 tab、滚到那张卡片并高亮,与目标引用同一个机制。另有一批内部整理与无障碍修正:三个目标型落地共用一道「挑一条指向目标的、其余记丢弃」的闸,意图框补视觉隐藏的标签与窄屏尺寸,「查看 Finding」与同行其它动作同形。处置反哺不设同目标互斥是有意的例外,理由写在代码与 `src/AGENTS.md` 上。详见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-08: 落地 issue #298(父 spec #293,ADR 0028)。**首次确认前也不用手写陈述:草案条目由一句话原地改写**。原来知识集还没确认时改一条草案条目只能自己填那张三字段表,陈述形状与 glob 全靠人把握。现在草案条目卡片上多一颗「改写」:人写一句话说要改成什么样,agent 拿到这条草案条目与这份草案里其余条目(后者让它别产一条与别处重复的陈述),读代码定作用范围,把那一行的型、陈述与作用范围原地换掉——标识不变、其余行不动、出处沿旧值。草案条目没有出处附注那张子表,这一次的来龙去脉只记在知识轨迹上,意图行记下动了哪一条。不指向这条草案条目的产出逐条丢掉并在轨迹里说明原因,一条都没有即零产出完成;陈述超过 100 字的照旧被那道闸丢掉。目标草案条目不存在即提交时 404(知识集已确认的仓库没有草案,那一档因此自然 404),同一条上已经跑着一条意图即 409;解读那几分钟里它被删掉、或者重新探索整组覆盖了这份草案的话意图改判失败并留原因,草案一字不动。面板:意图行多第三档「改写草案 #id」引用,点它切到知识草案 tab、滚到那张卡片并高亮,刚由意图动过的那几条带「刚由意图产出」徽章。草案的「修改」「新增」表单与「删除」照旧(撤直改是 issue #299)。详见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-08: 落地 issue #297(父 spec #293,ADR 0028)。**改一条生效知识条目也走裁决与出处**。原来改一条条目是直接填那张三字段表,改完当场生效,当时为什么这么改无处可查。现在条目卡片上多一颗「改写」:人写一句话说要改成什么样,agent 拿到这条条目的型、陈述、作用范围,**加上队列里已经指向它的那几条提案与它们的全部附注**,产出一条指向它的变更交人裁决——改陈述与作用范围的修改型、代码已经不支持它的废止型、或者换型的单目标合并型。**队列里已经有一条在等人裁决时优先并入那一条**:提示里点名让 agent 这么做,落地也只认这几条,人因此始终只裁决一次。不指向这条条目的产出逐条丢掉并在知识轨迹里说明原因,一条都没有即零产出完成——人说的那件事这条条目已经写到了,是预期结果。目标条目不存在或已废止即提交时 404,同一条条目上已经跑着一条意图即 409;解读那几分钟里它被人直接废止的话意图改判失败并留原因,队列一字不动。面板:意图行多一档「改写条目 #id」引用,点它切到知识条目 tab、滚到那张卡片并高亮,完成后它产出的那条提案在队列里带「刚由意图产出」徽章。「修改」与「废止」两颗按钮不动(撤直改是 issue #299,直接废止保留)。详见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-08: 落地 issue #296(父 spec #293,ADR 0028)。**处置备注成为一条以那条 Finding 为锚的修订意图,反哺由此有了运行状态**。原来带备注的处置回 200 之后另起一族后台函数解读,跑成什么样谁都看不见:失败只在服务端留一行日志,选不出模型连日志都只是「跳过」,写备注的人写完就再也见不到它去了哪里。现在处置落库那一刻先落一行修订意图——目标是那条 Finding、原文即备注、提交人即处置人、模型与档位沿反哺原来的规则,再走与人工提议同一个后台函数。**两条链路只在三处分叉**:工作副本停那条 Finding 报出时的 head(备注说的是那时的代码)而不是默认分支当前 head、agent 请求给反哺那一段而不是意图那一段(两段提示一字未改)、附注与知识轨迹的来源仍记处置反哺。粒度门槛、并入队列里已有的那一条、超长丢弃、模型规则与「解读失败不影响处置、处置响应不等它」全部照旧。反哺跑完写下一句收尾与产出标识,失败写进意图行,人在知识集弹窗顶部看得到它带原因的那一行、删得掉;**选不出模型那一档也落一行失败**,不再静默;服务重启时停在运行中的一并改判失败。无备注的处置照旧零触发,不建行也不排任务;处置那一侧的界面一字未改。面板的意图列表因此每行多一枚来源徽章,反哺那一行另多一颗「查看 Finding」走既有的侧滑。详见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-08: 落地 issue #295(父 spec #293,ADR 0028)。**待裁决的修订提案可以由一句话原地改写**。人在提案卡片上点「改写」写下要改成什么样,agent 拿到这条提案的变更类型、型、陈述、作用范围、目标条目与**全部出处附注**,只改这一条:陈述与作用范围换新,追加一条人工提议附注记下这段话与它的依据,队列里仍是那一条,人裁决一次。附注即修订对话——连续两次意图时第二次看得到第一次留下的那一条,不用从头解释。型沿既有映射:新增型可换型,修改型要换型即成单目标合并型(目标不变),合并型的型由新陈述定,废止型没有改写入口。目标不在待裁决队列里 404、废止型 400、同一目标已有意图在跑 409;落地那一刻它已被裁决即意图失败并留原因,提案一字不动。**目标型意图的公共机制随这一票落定**(目标校验、同目标互斥、目标内容进 agent 输入、「只改这一条」的提示、落地校验),目标为知识条目与草案条目的两票复用它。面板:待裁决卡片(废止型除外)多一颗「改写」就地展开与顶部同一个意图框,意图行带「改写提案 #id」引用,点它切到队列 tab 并高亮那张卡片。直改表单与「改后采纳」不动(撤直改是 issue #299)。详见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-08: 落地 issue #294(父 spec #293,ADR 0028)。**人在知识集弹窗写一段话,agent 读代码产出一条合形状的修订提案**。起因是知识集有五个直改面都让人自己填「型 + 陈述 + 作用范围 glob」那张表:陈述要合乎形状、glob 没有校验、型要在评审规则与项目事实之间判断,而 AI-API 的 75 条采纳里 47 条形状不合格——瓶颈在产出形状,不在交互轮数。这一票先立起入口:**修订意图成实体**,一行三态(运行中 / 失败 / 完成),带提交人、目标、模型与思考档位、知识轨迹、产出标识与一句收尾,每仓库多行,行永久保留而弹窗只列运行中、失败与刚完成十分钟内的。提交要 `knowledge:write`,原文去空白后 1 到 500 字,选不出模型即 409 并说清去配凭据或先跑一次基点探索;失败行与完成行可删,运行中删不掉——取消一次运行不在这一票里。后台那一步与处置反哺同一族:模型沿反哺规则(该仓库最近一次基点探索的那一组,否则全局模型组合第一个),**工作树停默认分支当前 head**(意图说的是这个仓库现在的样子,不是某一次 Finding 报出时的那个 commit),产出经与反哺同一套映射入队——认出队列里已有同一件事即并入那一条,知识集未确认时追加进草案(追加不覆盖,整组覆盖只属于重新探索)。**人工提议是出处附注与知识轨迹的第四种来源**,旧库照既有「判据 + 原地重建」演进,存量的「手工」出处一条不改;每条产出的附注备注原文记意图、依据记 agent 的理由,人裁决时展开就读得到它凭什么成立。零产出是完成并写「未产出变更」,异常即失败写进意图行、不重试,服务重启时运行中的改判失败。意图不受基点探索与知识整理的互斥限制,各派自己的工作树。面板:弹窗顶部一个带 500 字计数的意图框(无权限不显示、无模型置灰)、一列带状态、轨迹入口与删除的意图行、产出提案卡片上一枚「刚由意图产出」徽章,有意图在跑时五秒轮询。本票只做无目标的意图;目标型三票与撤直改在后。详见 `src/AGENTS.md` 与 `web/AGENTS.md`。

- 2026-09-08: 落地 spec #286(issue #287–#291)。**知识条目的陈述有了形状,证据从陈述分离到附注,评审规则扩为两类**。起因是 AI-API 把处置反哺的产出全部裁决进知识集之后,75 条里 47 条是「这条 Finding 为什么错」的论证:一句均长 252 字、带行号与调用点、以「因此不作为缺陷」收尾,粒度是单条 Finding。五票:#287 出处附注多一格依据,三条链路的 reason 都落进去;#288 Reviewer 的规则段认得两类规则(对代码的要求违反即 Finding,对评审的限定只收窄报什么、不产 Finding);#289 合并型放宽到单目标即改型;#290 三条链路共用一段陈述形状(一句不超过 100 字、写不变量、不带证据与结论句),服务端按 100 字拦,反哺加粒度门槛;#291 知识整理按形状对现集提改写、合并与改型提案。评审修复:整理直改超长时轨迹仍记那一条 `rule_consolidated`(没落地、带原因);`readProposalType` 让写错的 `type` 在单目标上取目标的型,免得一次裁决误改型。CONTEXT.md 新词条「陈述形状」,评审规则、项目事实、出处附注、修订提案、处置反哺、知识整理、裁决七个词条改口;ADR 0019 加修订附记。详见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-08: **增量评审弹窗的「完整审查」默认改为不勾**,与重跑弹窗同一个默认:推进出来的那一轮默认只复核这个阶段未处置的历史,要审新代码时勾上。只改面板默认值,`POST /range-reviews/:id/advance` 不带 `mode` 仍是完整审查。CONTEXT.md 只复核词条改口:重跑与增量评审默认都是只复核。详见 `web/AGENTS.md`。
- 2026-09-08: spec #280 那一批(issue #281–#285)的两轴评审修复。**三处:整理在跑时那句 409 说得出跑的是整理、修订提案那一页在现集非空时也出现、CONTEXT.md 的出处附注词条不再把 `source` 列为忌用词。**发起基点探索被挡下时,回执原来一律说「已经有一次基点探索在跑」——互斥自 issue #284 起跨探索与整理两条链路判,整理在跑时这句话把人指错了地方;现在两侧的回执同一形状,各自说得出在跑的是哪一个。面板那一侧:知识整理的入口挂在修订提案那颗 tab 上,而那颗 tab 原来只在队列非空时出现,「现集非空、队列为空」这个整理本来跑得动的局面下人根本发起不了;现在有 `knowledge:write` 而现集非空时那颗 tab 同样出现,队列空着就是一句空态。另有两处内部整理:两条发起链路四十行逐字相同的前置校验收成一份,`source` 是代码里全程在用的标识(`rule_proposal_source` 表、`review_trace.source`),忌用词表里不该有它。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-08: 落地 issue #283(父 spec #280,ADR 0019)。**处置反哺认出队列里已有同一件事就并入那一条,不再排第二条**。原来每条处置备注各排各的提案:两个人对同一件事各写一句,队列里就是两条,人要读两遍、裁两次,采纳第一条之后第二条的目标还可能已经失效。现在解读那一步除现集之外还拿到这个仓库的待裁决队列(每条带标识、变更类型、目标与陈述),agent 认出这条备注说的是队列里已有的一件事时指名并入那一条:队列里那条的陈述换成合成两次说法之后的新陈述,并追加一条处置反哺附注(备注原文、这次的 Finding、这次的知识轨迹)。**作用范围与型不随并入改**——agent 手上没有队列那条的作用范围,拿一份只看新备注写出的 glob 去覆盖会把它缩到说不上话的范围;型不动是不让一条规则悄悄变成不产 Finding 的事实。指名的提案已裁决或根本不存在时那一条退回按新增处理,并入缺陈述时那一条丢掉、轨迹里留一行原因。并入后的提案带着处置反哺附注,下一次重探索取代不掉它。规则集弹窗上展开这条提案即看得到两条附注与两条 Finding 引用(issue #281 的出处附注列表)。`CONTEXT.md` 的处置反哺词条补这一句。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-08: 落地 issue #285(父 spec #280,ADR 0019)。**知识整理在直改队列之外,还对现集提出变更提案**。整理 agent 一次读完全部生效条目,因此看得见知识集自己积下的重复与矛盾:两条说同一件事的条目提一条合并提案,措辞会被读错的提一条修改提案,与别的条目矛盾而站不住的提一条废止提案。这些提案与另两条链路进同一个队列、走同一套映射与同一组裁决按钮,各带一条出处附注:来源记知识整理,备注是 agent 的理由加它涉及的条目,轨迹回到这一次整理。**知识集仍然只由裁决改动**:整理直改的只有还没人裁决的队列,对现集的变更一律排队等人;目标条目在落地那一刻不生效的,丢弃那一条。重探索不取代这些提案(它推不出人与整理认出来的东西)。采纳一条整理提出的合并提案即几条目标全部废止于新版、合成的那一条生效于同一版,出处记知识整理。完成后的摘要多一段,变成「合并 n 条提案、改写 m 条、提出 k 条」。现集与待裁决队列都空时整理短路,不跑 agent(面板那一侧当天的评审修复:修订提案 tab 因此在现集非空时也出现,整理入口跟着可达)。CONTEXT.md 的知识整理词条随之补上「也对现集提变更」。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-08: 落地 issue #282(父 spec #280,ADR 0019)。**修订提案多一种变更类型「合并」:几条说同一件事的知识条目换成一条新陈述**。原来只有新增 / 修改 / 废止三档,几条重复条目要人一条条废止再新增一条,一次合并裁决三四回,中途每采纳一次还会让剩下几条的目标失效。现在规则 agent 认出重复即提一条合并提案:目标参数改为可给多个,给了两条以上认得出的目标就是合并型,认不出的目标先逐个丢掉——剩一条即退化为修改或废止,一条不剩即新增,带废止标记而认得出的目标不止一条的整条丢掉(废止一次只废止一条)。采纳一条合并提案就把几条目标一起废止于新版、合成的那一条生效于同一版,只推进一个知识集版本,出处记提案自己的;目标里有一条已经不生效就采纳不了,回 404 且与修改型同一句话,驳回照常只改状态。采纳前照样改得动内容,改后采纳同样成立。规则集弹窗的合并型卡片列出全部目标条目的陈述与作用范围,有 `knowledge:write` 时可改、可采纳、可驳回。存量提案的单值目标列迁移成一元的目标列表,新增那些是空列表。`CONTEXT.md` 的修订提案与裁决两个词条补合并语义,ADR 0019 加一段修订。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-08: 落地 issue #284(父 spec #280,ADR 0019)。**知识整理:在规则集弹窗里手动发起一次 agent 任务,直接整理待裁决队列**。有 `knowledge:write` 的人在修订提案那一页选一个模型发起,与基点探索共用「同仓库同时只跑一个」,另一个在跑时回 409。整理 agent 读当前生效的知识集与待裁决队列(连标识、变更类型、目标、陈述与出处附注),不读代码、不派生工作树,产出两类直改:说同一件事的几条提案合成一条(保留标识最小的那一行、其余删除、附注全部并入保留行、陈述换成合成后的那一句),现集已经有的新增型提案改写成指向那条条目的修改型(评审人因此一眼看得出它与现集的差别,内容一样就驳回)。**整理不冻结任何人的工作**:整理期间裁决与处置照常,产出落地时逐条校验——被并的提案不再待裁决即跳过那一次合并,目标条目不生效即丢弃那一条改写。运行状态与基点探索同形:运行中 / 失败 / 完成三态、失败原因、轨迹入口(与探索同一张轨迹表,每个动作留一条事件并写明落没落地),完成后一句「合并 n 条提案、改写 m 条」。进程重启时停在运行中的整理改判失败。它不裁决,也不改知识集:知识集仍然只由人的裁决改动。CONTEXT.md 新增「知识整理」词条并改写「知识轨迹」,ADR 0019 加一段修订。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-08: 落地 issue #281(父 spec #280,ADR 0019)。**一条修订提案的出处从单值变成一列出处附注**:每条附注自带来源(基点探索 / 处置反哺 / 知识整理)、备注原文、引发它的那条 Finding 与那一次任务的知识轨迹,一条提案至少一条,提案的出处即附注全体。存量提案的旧三列各合成一条附注,来源、备注与轨迹逐字可回读,那三列随之从表上去掉。基点探索的产出各带一条基点探索附注,处置反哺的各带一条处置反哺附注(备注原文与那条 Finding 都在上面)。**重探索的取代范围随之收窄**:只取代待裁决且附注全部来自基点探索的提案,带处置备注来源的留下——人写的意见不该被一次重探索抹掉。规则集弹窗的提案卡片出处一行汇总、展开逐条可读,Finding 引用走既有的 `?finding=` 侧滑;没有 `knowledge:write` 的人照旧读得到队列与出处。这是 spec #280 其余各票(反哺并入、知识整理、合并变更类型)的前置改造:一条提案上从此挂得下多个来源。`CONTEXT.md` 的修订提案词条改写并新增「出处附注」,ADR 0019 加一段修订。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-07: 落地 issue #279(父 spec #277)。**几个模型报同一处时,那一份正文由合并 agent 综合各家的说法写出**:issue #278 之后正文只剩一份,但那一份是某一个模型的原话,另外几个模型看到的角度要读的人自己去可展开区里拼。现在判分组的那个 agent 在给出分组的同时,为每个多归属的组写一份标题、问题、影响与建议——合各家说过的,不加谁都没说过的;严重度与分类仍由代码定(最高 / 首报),它写的文本改不了处置口径。归属只有一条的组不劳它,原文就是正文。它某一组没写、或写出来标题与问题说明是空的,那一组退回原文最完整的一段,分组照收、评论照发,审查轨迹上留一条写明是第几组、为什么。整轮跑不成或分组不过验收时,合并退回算法档,正文一律用原文最完整的那一段。Forge 评论与面板的样子不变。
- 2026-09-07: 落地 issue #278(父 spec #277)。**几个模型报同一处时,正文只有一份说法,模型名退到末尾一行**:此前一条 Finding 的正文按归属逐段堆叠,同一个模型跨批次重复报出同一处会让面板上出现六段几乎同义的「问题 / 影响 / 建议」,每段都顶着模型名。现在合并组派生一份代表段——取描述最长的那条归属的标题、问题、影响与建议,四段同出一条,严重度仍取最高、分类仍取首报;Gitea 评论正文是这三段加末尾一行「由 N 个模型报出:a、b」,不带各模型原文,延续那一句、承接段与指纹锚点位置照旧;面板正文同一份,全部归属与承接来的历史说法收在「各模型原文」折叠区里,只有一条归属又没有承接段时不给这个入口。代表段随 Finding 落库(`finding` 表加影响与建议两列),升级前的行两列为 NULL,面板按同一规则从归属现算,已发出的评论不改。ADR 0015 补一段修订,CONTEXT.md 的「Finding」词条改写并新增「综合说明」词条。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-07: 落地 issue #276(父 spec #269)。**只复核被拒之前先清掉回退或删除文件上的未处置历史**:这些条目谁都复核不到,而只复核又按「有没有可复核的东西」准入——一个阶段的未处置历史全落在它们上面时,只复核被拒、轮次不开,开跑时的自动处置永远走不到,评审人只能为了清它们专门开一轮完整审查。现在 PR 重跑、范围审查重跑与增量评审推进三处在拒绝之前先按开跑那一步的同一份判据把它们自动处置掉(Gitea 上的评论一并 resolve,处置备注仍是「文件已回退 / 已删除,自动处置」;Gitea 的变更文件清单为改名条目补出旧路径的删除,落在改名前路径上的历史因此与本地 diff 那一侧同样判「文件已删除」);处置完还有可复核历史就照常开轮,仍然没有就回原来那句拒绝,前面加上「已自动处置 N 条回退或删除文件上的历史」。被拒时除这一次处置之外一字不变:比较项、容器 PR 的 head、来源与推进人都停在拒绝之前那一刻。完整审查的准入不变。细节见 `src/AGENTS.md`。
- 2026-09-07: 修 issue #275。**同一处的多条评论自动处置时不再漏掉其中几条**:同一轮里落在同一文件同一处的两条 Finding 各带一条 Gitea 评论时,下一轮复核判已修(或文件被删除 / 回退)只 resolve 了其中一条,另一条评论留在 Gitea 上未处置,而面板上两条都已记成「已修复」。现在这一处上的每一条评论都逐条写回 Forge,写成一条才记一条;只活在 review 正文里、没有评论载体的那些保持未处置,不再被顺手记成已修复。写 Forge 失败的照旧留给人处置。细节见 `src/AGENTS.md`。
- 2026-09-07: 落地 issue #274(父 spec #269)。**阶段详情可以一键处置低于最低报告等级的未处置项**:阈值抬高之后阶段里还留着一批按旧阈值报出来的低等级条目,工具行的新按钮弹二次确认(显示将处置的条数与一格可选备注),确认后按这个阶段的生效阈值逐条标记为人工已处置并同步 resolve 掉 Forge 上的评论;逐条执行,一条失败不中断,面板列出失败条数、可以再点一次。仓库的生效阈值是 P2 时按钮不出现,端点回 409。新权限格 `finding:dispose-batch`,逐条处置的 `finding:dispose` 不包含它。
- 2026-09-07: 落地 issue #273(父 spec #269)。**最低报告等级可按仓库覆盖**:仓库配置弹窗多一块「最低报告等级」,与模型组合同形的「跟随全局 / 自定义」两态——点「跟随全局」直接清掉覆盖,点「自定义」选 P0 / P1 / P2。仓库注册表多一列可空覆盖,不设即跟随审查策略里的那一档,再没有即默认 P2(全部报出)。生效值仍只有一个入口,开跑那一刻取一次并随轮次落库,改配置只影响下一轮。新增 `PUT /repos/{repoId}/min-report-severity`(权限 `repo:write`,取值不合法 400),`GET /repos` 每行带出这个仓库的覆盖与眼下的全局档。审查轨迹补上「开跑时自动处置历史」那一档的呈现(issue #272 落的库)。CONTEXT.md 的「最低报告等级」与「审查策略」两条词条随之写明仓库覆盖。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-07: 落地 issue #272。**所在文件已回退或已删除的未处置历史在开跑时自动处置**:每一轮 Review Run 读完历史之后,未处置历史所在的文件在这一轮 diff 里被删、或根本不在 diff 里(代码已回到 base)时,该条直接自动 resolve 并记为「已修复」,处置备注区分「文件已删除」与「文件已回退」,轨迹记一条对应事件;写 Forge 失败的照旧留给人处置。这些条目此前谁都复核不到,只会永远悬在未处置列表里。完整审查与只复核同律,PR 审查与范围审查同律。细节见 `src/AGENTS.md`。
- 2026-09-07: 落地 issue #271(父 spec #269)。**审查策略新增全局「最低报告等级」**:取值 P0 / P1 / P2,默认 P2(全部报出),与四项上限同形独立读写(来源、版本冲突 409、恢复系统默认)。阈值非 P2 时 Reviewer 的 prompt 多一段告知它不要报低于阈值的,合并去重之前再按归一后的严重度过滤一次保底,丢弃条数记一条 `findings_filtered` 轨迹事件;被丢的不落 Finding、不进 Forge、不参与指纹折叠。阈值只管新报——未处置的历史 Finding 一律照旧注入并复核。每一轮开跑时读一次并随轮次落库,改设置只影响下一轮,续跑核对它。CONTEXT.md 新增「最低报告等级」词条。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
- 2026-09-06: 落地 issue #268。**恢复已有评审记录中可追溯的影响与修改建议**:新增一次性操作入口 `src/recover-finding-content.ts`(见「常用命令」),默认只读预览,`--apply` 先留库快照再一个事务写入。来源只认唯一对应:同一轮同一模型对同一文件同一段问题表述、成功上报的轨迹内容恰好一种才补,两种即跳过;同时核对能确认的原评论(只认这一行自己发布的评论,折叠到旧评论上的行不算),来源或已有内容矛盾时跳过;按复核结论合成的延续先识别,再沿可追溯的延续关系从上一处抄历史说法,判据或来源不足时列明原因。只补 NULL,非空不碰;不调用模型、不重跑、不写 Forge。细节见 `src/AGENTS.md`。
- 2026-09-06: 落地 issue #267。**历史 Finding 延续时保留影响、修改建议及出处**:复核判仍在、代码位置已变而本轮没有重报时,合成的延续 Finding 把历史各模型归属的问题、影响与建议原样带到新位置,每段记原模型与来源轮次的 head;新 Gitea 评论与面板都标注「沿用 X 在 Review Run #N / <head> 上的说法,尚未针对新代码重新验证」,不署给给出新位置的模型,也不算进它的归属与统计;本轮重报的用本轮内容。历史说法另落一张表,多轮延续出处仍是最初那一轮。细节见 `src/AGENTS.md` 与 `web/AGENTS.md`。
