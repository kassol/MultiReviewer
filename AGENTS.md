...
- 2026-09-09: 收口 issue #307 的写侧(父 spec #305,ADR 0030)。**同一处的两条 Finding 各自记账,互不波及**:同一处未改动代码上先后报出的两个不同问题,此前在回填、自动处置、延续与交接四处仍按「文件 + 指纹」认作一条——上一轮那条被驳回,下一轮回填就把「已处置」写到同一处那条新问题上,它于是无声无息地关掉,而人从没看过它。四处改按承载它的那条 Forge 评论认账,与阶段统计同一个键;没有评论承载的行(只活在 review 正文里的)仍按「文件 + 指纹」。对人的影响:同一处的新问题从此保持未处置、留在待办里,直到有人真的处置它;单条 Finding 的行为、处置率与参与条数的口径一字未动。不加列、不加迁移,旧库直接打开。
