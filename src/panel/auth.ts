...
/** 面板的登录判定。形状跟着 `createPanelAuth` 走,调用方按它标注。 */
export type PanelAuth = ReturnType<typeof createPanelAuth>;
...
/** 一次登录判定:退避闸门、Argon2 比对与失败计数都在这个闭包里,进程内一份。 */
export function createPanelAuth(
  options: {
    now?: () => number;
    onGate?: (event: { account: string; ip: string; count: number }) => void;
  } = {},
) {
  ...
  return async function login(
    ...
    { ok: true } | { ok: false; status: 401 } | { ok: false; status: 429; retryAfter: number }
  > {
