...
Report only what the note itself justifies. A note that settles this one finding and nothing more justifies no change at all — reporting nothing is an expected outcome. A note that only says the code was changed or the finding was fixed is such a note: it records a repair, not a standard, so report nothing. Read the code around the finding when you need it to tell a one-off from a standing rule, or to check a fact before stating it; the evidence you read goes in the reason field, not in the statement.
