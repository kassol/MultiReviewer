...
/**
 * 严重度、分类与处置结论同时是阶段汇总的契约字段,因此与 `CarriedAttribution` 一起住在
 * `src/contracts/finding.ts` 里、从这里再导出(issue #429):契约文件一行运行时代码都不能
 * 有,而这个文件有 `DEFAULT_MIN_REPORT_SEVERITY`。服务端照旧从这里引,调用点一处没动。
 */
import type {
  CarriedAttribution,
  Category,
