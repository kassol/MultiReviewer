 ...
 * 不处置就永远悬在未处置列表里。
 *
 * 原因看它还在不在这一次的变更文件清单里:在,那它只能是被这一次删掉的——改名的旧路径
 * 也在内,两条来源都为它落一条 `removed`(本地 diff 关了重命名检测,Gitea 那侧按
 * `previous_filename` 补,issue #276);不在,即代码已经回到 base 状态。
 *
 ...
 * 文件只认新路径——两种来源都把改名摊成旧路径删除加新路径新增(本地 diff 关了重命名
 * 检测,Gitea 那侧的 `renamed` 条目按 `previous_filename` 补出旧路径,issue #276),
 * 过滤掉删除那一半之后剩下的正是新路径。
 *
