...
export function runTrackerRequest(
  ...
  const specOf = (specId: number): ProductSpecRecord | undefined => {
    const spec = store.getProductSpec(specId);
    return spec?.productId === productId ? spec : undefined;
  };
  /** 这个产品下的一张票。跨产品的边正是由这一判打回的。 */
  const ticketOf = (ticketId: number): ProductTicketRecord | undefined => {
    const ticket = store.getProductTicket(ticketId);
  ...
  switch (request.kind) {
    case "create-spec": {
      ...
      const spec = store.createProductSpec({ productId, title, body, sessionId, at });
    ...
    case "create-ticket": {
      ...
      if (specOf(request.specId) === undefined) return noSpec(request.specId);
    ...
    case "list": {
      const specs = store.listProductSpecs(productId);
      const tickets = store.listProductTickets(productId);
      if (specs.length === 0) {
    ...
    case "read": {
      ...
      const ticket = ticketOf(target.id);
      if (ticket === undefined) return noTicket(target.id);
      const comments = store.listProductTicketComments(ticket.id);
      return [
    ...
    case "update-body": {
      ...
      if (target.kind === "spec") {
        if (specOf(target.id) === undefined) return noSpec(target.id);
        store.setProductSpecBody(target.id, body);
      } else {
        if (ticketOf(target.id) === undefined) return noTicket(target.id);
        store.setProductTicketBody(target.id, body);
      }
    ...
    case "close": {
      ...
      if (target.kind === "spec") {
        if (specOf(target.id) === undefined) return noSpec(target.id);
        return store.setProductSpecState(target.id, "closed", at)
          ? `spec ${target.id} is closed`
          : `spec ${target.id} was already closed`;
      }
      if (ticketOf(target.id) === undefined) return noTicket(target.id);
      return store.setProductTicketState(target.id, "closed", at)
        ? `ticket ${target.id} is closed`
        : `ticket ${target.id} was already closed`;
    }
    case "comment": {
      const body = request.body.trim();
      if (ticketOf(request.ticketId) === undefined) return noTicket(request.ticketId);
    ...
    case "unblock": {
      ...
      if (ticketOf(ticketId) === undefined) return noTicket(ticketId);
      ...
      if (ticketOf(blockedById) === undefined) return noTicket(blockedById);
      if (request.kind === "block") {
        store.addProductTicketBlock(ticketId, blockedById);
        return `ticket ${ticketId} is now blocked by ticket ${blockedById}`;
      }
      return store.removeProductTicketBlock(ticketId, blockedById)
