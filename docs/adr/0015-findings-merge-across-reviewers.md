...
正文的呈现单位从归属改为一份代表段(issue #278)。合并组落库与呈现的标题、问题、影响与建议取描述最长的那条归属,四段同出一条;Forge 评论正文是这三段加末尾一行「由 N 个模型报出:a、b」,不带各模型原文;面板正文同一份,全部归属与延续承接来的历史说法折进「各模型原文」折叠区。归属全保留仍然成立,2026-08-31 那次修订不变:同一个模型内容不同的几条仍各占一条归属,只是它们不再各自成段占据正文。理由是线上实况——同一个模型跨批次报出同一处会留下六条归属,正文因此堆着六份几乎同义的说法,模型名反倒占着最显眼的位置;人要读的是一份说清楚的问题、影响与建议,谁报的是归属信息。严重度仍取最高、分类仍取首报,代表段的选择不参与处置口径。`finding` 表加 `impact` / `suggestion` 两列存代表段的后两段;升级前落的行两列是 NULL,读侧按同一规则从归属现算,不回填,已发出的评论不改。
