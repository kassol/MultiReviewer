 ...
 */
function rewriteHead(ctx: Awaited<ReturnType<typeof setup>>, source: string): void {
...
test("分组方案过验收即生效:同一行的两个问题被拆开,相邻的同一个问题被合并", async () => {
  ...
  const events = await trace(db.url);
...
test("单成员组不产生合并事件", async () => {
  ...
  assert.equal(result.findings.length, 2);
  assert.equal((await trace(db.url)).filter((event) => event.kind === "finding_merged").length, 0);
});
...
async function assertFallback(
  ...
  findings: readonly Awaited<ReturnType<typeof AT>>[] = [AT(2, "删除了余额校验"), AT(3, "余额校验被删了")],
  ...
  const fallbacks = (await trace(db.url)).filter((event) => event.kind === "merge_fallback");
...
test("多归属组的综合说明成为正文,归属保留各模型原话", async () => {
  ...
  assert.equal((await trace(db.url)).filter((e) => e.kind === "synthesis_fallback").length, 0);
});
...
test("缺综合的那一组退回代表段,其余组照用综合,轨迹记一条 synthesis_fallback", async () => {
  ...
  const events = (await trace(db.url)).filter((e) => e.kind === "synthesis_fallback");
  ...
  assert.equal((await trace(db.url)).filter((e) => e.kind === "merge_fallback").length, 0);
});
...
async function dispose(databaseUrl: string, forge: MemoryForge, commentId: string): Promise<void> {
  const store = openStore(databaseUrl);
...
async function firstRun(
  ...
  findings: readonly Awaited<ReturnType<typeof AT>>[],
): Promise<void> {
...
function carryComments(ctx: Awaited<ReturnType<typeof setup>>): void {
...
test("合并请求里的历史只含本轮有 Finding 的文件,未处置与已处置都在", async () => {
  ...
  await dispose(ctx.db.url, ctx.forge, "comment-1");
...
test("agent 把本轮一条与旧指纹仍在的历史分成一组:不发评论,折叠到旧条,判据是 agent", async () => {
  ...
  const folded = (await trace(ctx.db.url)).filter((event) => event.kind === "finding_folded");
...
test("含历史成员的组行距超容差仍过验收", async () => {
  ...
  assert.equal((await trace(ctx.db.url)).filter((e) => e.kind === "merge_fallback").length, 0);
  assert.equal(result.findings.length, 1);
});
...
test("同一条历史被分进两组的方案整体作废回退", async () => {
  ...
  const fallbacks = (await trace(ctx.db.url)).filter((e) => e.kind === "merge_fallback");
...
test("命中的历史所指代码已改写:走延续,旧评论 resolve,新评论带「延续自」", async () => {
  ...
  const continued = (await trace(ctx.db.url)).filter((event) => event.kind === "finding_continued");
...
test("命中的历史本轮已被全部 Reviewer 判已修:不延续,旧行留「已修复」,本轮那条是新 Finding", async () => {
  ...
  assert.equal(
    (await trace(ctx.db.url)).filter((event) => event.kind === "finding_continued").length,
    0,
    "轨迹不记延续",
  );
});
...
test("同一处的另一个问题:agent 只把其中一条归给历史,另一条发新评论且未处置", async () => {
  ...
  const notFolded = (await trace(ctx.db.url)).filter((event) => event.kind === "finding_not_folded");
...
test("合并 agent 不可用的那一轮:同一处的新问题仍按指纹并进旧条,轨迹记回退", async () => {
  ...
  const events = await trace(ctx.db.url);
...
async function twoAtOneSpot(
  ...
  await firstRun(ctx, [AT(2, "余额校验被删掉")]);
  if (options.disposeA === true) await dispose(ctx.db.url, ctx.forge, "comment-1");
  const merge = scriptedMergeAgent((request) => [
    { members: [0], history: [request.history![0]!.id], reason: "还是那处余额校验" },
    { members: [1], reason: "日志里打印密钥是另一个问题" },
  ]);
...
test("所在文件回退:同一处的两条各自自动处置,两条评论都 resolve", async () => {
  ...
  const traced = (await trace(ctx.db.url)).find((event) => event.kind === "history_auto_disposed");
